...
**决策点**：
- D2-1 走路口径：膝"自然走10米"、踝"走一小段"、局部"以平时速度走一小段"——评估统一成"固定距离"还是保留部位差异？（建议：评估统一 10 米，功能复测沿用原句；需 owner 一句话）
- D2-2 症状页选择器措辞变化（髋四方向改词根叫法）是否接受。
...
| D2-1 | 走路评估口径（10米 vs 一小段） | 2 | 评估统一 10 米 |
| D2-2 | 症状页选择器改词根叫法 | 2 | 接受（暂停点过目） |
