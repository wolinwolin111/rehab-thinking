...
| 15 | （无主题档，记录在 continuation §6） | `cfb2dbd`（agent/dev-20260901 tip：e2a00f8 batch2②③④ + 2fbce9c app-shell 修复 + a2b264a 通知 + d33c874 边界修复 + 0e9cc34/1b55396 custom-action 靶子 + cfb2dbd 通知） | CA-1/CA-2 新增；app-shell 转绿；边界转绿；reg 96；基线 68+1红→**71+0** | dev batch2 custom-action 绑定闭环：app-shell 真回归经 dev 2fbce9c（pointer-events:none）修复重跑绿；e2a00f8 引入的 assessment-stage 直连 domain 边界违规经 d33c874（走 stage-domain-adapters）修复转绿；custom-action 卡两分支（默认四档负荷 / 急性+肿胀硬闸抑制现场复现）用 dev catalog 靶子 0e9cc34/1b55396 + launchWorkbenchScenario 稳定覆盖（探针实测落 step2 即聚焦卡）；干净全绿 run reg-20260902210613-27712 |
| 16 | （无主题档，记录在 continuation §6） | `228a9aa`（agent/dev-20260901 tip：b67ee5e C-3 三圈下段截短 + 228a9aa 前侧嵴外移/后侧平滑/四圈 reviewed） | C3-CALF-OVERLAY 源码契约新增；reg 97；基线 71+0 保持（node 787→788） | C-3 小腿四圈解剖复核绑定：纯视觉叠加（MUSCLE_ZONE_PATHS path 几何），不改选择/文案/字段；钉唯一机器可校验契约——data-overlay-review 接线 + 四小腿圈 reviewed（搜索域限定对象内防跨块误抓 thigh pending，只锁 calf 避迁移税）；path 几何不做像素断言（§8），渲染无错由既有 decision-gates（进肌肉紧张度页+assertNoRuntimeErrors）兜底；dev 无头渲染 PNG 逐卡判读闭环；干净全绿 run reg-20260903033829-16244 |

## 提交历史线（agent/testing，当前 tip `80921ed`（merge dev 228a9aa）；docs 提交持续前移，权威绑定见 continuation-handoff §6）
...
- **当前基线以 `test-workflow-continuation-handoff.md` §6 为唯一权威**（当前：Edge full 71 passed + 0 failed、mobile 2/2、node 788/0、registry 97 条、run `reg-20260903033829-16244`）。本索引不再维护平行计数，避免漂移。
