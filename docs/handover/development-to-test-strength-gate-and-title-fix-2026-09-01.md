...
## 7. 复核修复（批 G 自审后追加，fix-forward）

对 `8411e33`/`fff8951` 的对抗性复核发现并修复两处批 G 引入的问题：

1. **续测接受项静默丢失（回归）**：补查建议池取区域力量全集，但装配从**闸门后**的 `strengthItems` 里找已接受项——用户接受被闸门裁掉的力量补查时，该项不出现且无提示。修复：`strengthItems` 过滤加 `|| continuationRoundIds.includes("strength:"+id)` 旁路。既有同类坑（方向项经 `directionIsRelevant`/能力闸门后同样丢失，踝区早于批 G 存在）未在本批处理，测试侧若钉"接受补查必出现"契约，方向侧仍会红——那是既有缺陷，值得开条目。
2. **腘绳触发词与决策核不一致**：`pilot-decision-engine` :381 按症状词（屈膝/弯膝发力/脚跟向后拉/膝后无力/大腿后侧无力）点名 `strength:knee-hamstring`，批 G 闸门只认位置词（膝后/腘窝/大腿后侧），膝前痛+屈膝无力的用户会被裁掉决策核点名的检查（有 availableIds 守卫不崩，但意图被静默丢弃）。修复：闸门为腘绳增加与决策核同口径的症状词旁路。

复核确认无问题的链路：膝区处理候选 retestIds 不引用被裁力量项；`KNEE_PAGE_ASSESSMENTS` 夹具未用键自动忽略；`adaptExerciseForCurrentStage` 对象展开保留 purpose；批 H pilotOnly 六项排除集精确（有其它合法来源的卡不受影响）；改名后旧标题全库无残留引用。验证：`npx tsx` 6 用例全过（含回归确认），typecheck 干净，组件测试与基线同红零新增。
