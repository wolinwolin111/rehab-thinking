...
- **R-06 部分完成（U02 更正）**：弃用 `body.style.zoom`，改**布局视口折半＋DPR 翻倍**——该方法验证的是**极窄视口重排**（195/160px），不是真实浏览器 125%/200% 文字放大；重排验收发现并修复 3 个真问题（zoom 法测不出）：①极窄视口顶栏 logo/状态/按钮叠印 → 状态行隐藏＋顶栏压缩；②入门教程卡溢出视口且"跳过教程"不可达（卡死）→ 卡片钳制在视口内；③评分卡在 195px 横向出界 → 卡内堆叠＋min-width 解除。**测量口径修正**：发现 body 是 overflow:hidden、滚动发生在内部容器——此前 window.scrollTo 判据全部无效，已改为探测真实滚动容器；末元素可达性改为"叶子矩形与 rail 矩形不相交"（与 Toast 同几何口径）。**真实 125%/200% 文字放大验收仍未完成**，不得引用本文宣称其通过。
- **R-07 已登记**：见下"测试侧请求"。
...
### 真实浏览器
> **证据口径更正（U02，2026-09-09 全面整改单 §2.1 实测指出）**：下述 200% 结论的截图存在文件名与业务页错位——`step6-summary.png` 实为训练/统一复测页（非总结页证据）、`step5-training.png` 实为过渡卡；第一组训练快速反馈直接渲染在 `.rm-first-set`，并未藏在 details 折叠区（此前"反馈在 details 内"的说法有误）；viewport 折半＋DPR2 只能验证窄视口重排，不能等同真实浏览器 125%/200% 文字放大验收。**修正后的状态**：200% 方法验证过的是"极窄视口重排"（195/160px），在该范围发现并修复顶栏叠印、教程卡死、评分卡出界；"真实文字放大全流程通过"**未验证，撤回**。重排的正式口径是 320 CSS px 等要求，160/195px 仅是额外压力检查。
- 125%：仅前段采样，不宣称全流程
- 200%（实为 195/160px 压力视口重排）：修复前暴露顶栏叠印、教程卡死、评分卡出界三个真缺陷，均已修复并复测；**不作为真实文字放大验收依据**
- 320/390 全部步骤：overflowX=0（桌面模拟）
- 844×390：横屏浅色总结条件已入统一媒体（CSS 层），目视帧待补采
- 真机（iOS/Android 键盘、安全区、系统字号）：**待补验证**，本轮无真机结论
