...
# 第 30 轮 — 双视角走查 2 缺陷：dev 核对确认＋修复（回应测试侧）

## 缺陷 1（松解时长双轨）：确认，且比报告的更严重

核对结论：owner 裁定 f9721b6（松解类 30～60→60～90）**只落在了目录数据层**（TREATMENT_ENTRIES dose，golden 锁的就是它）；而**全部活渲染路径都没吃到裁定**——膝治疗卡走 `knee-workflow-adapter.ts`（8 处 30～60）、肌肉候选兜底剂量走 `candidate-action-core.ts:58`、`full-demo-content.ts` 19 处内联、`local-limb-regions.ts` 9 处、`build-trial-targets-core.ts` 双侧 2 处、`home-relaxation-core.ts` 居家放松 1 处、`summary-stage.tsx` 训练前准备 1 处、`workbench-support.tsx` 双侧兜底 1 处。批次 7"逐字搬进目录"的 `KNEE_TREATMENT_INSTRUCTIONS`/`KNEE_LATERAL_CHAIN_ANTERIOR` 是**零消费者死镜像**（当时未接线），它把 30～60 又抄进目录文件造成"目录内双轨"观感。裁定"30-60 太少"是临床口径，逐字保真是搬运纪律——前者赢。

修复（本批提交）：

- 上述全部活路径 30～60秒→60～90秒（含死镜像同步，防再被接线时复活旧口径）。
- `candidate-action-core.ts:58` 兜底剂量改为「保持60～90秒。」——**删去"必要时一次延长至90秒"**（90 已是上限，保留会自相矛盾；未发明 120 等新剂量，最小忠实于裁定）。
- `check-action-catalog.ts` 新增护栏 `CAT-RELEASE-DOSE-DRIFT`：src 任何 .ts/.tsx 出现「30～60秒」即构建失败（探针文件实测红→绿，防任何路径复活旧口径）。
- 双侧松解/居家放松/训练前准备三处按"松解类"并入裁定范围——若 owner 认为这三处（尤其居家放松 1～2 轮）不该跟涨，回滚只需单点改字符串。

## 缺陷 2（不支持区死胡同）：确认，三点逐一回应

根因核对全部属实：`unsupportedDescriptionRegion` 由 `inferRegion(描述)` 动态得出（workbench:3341），但提示卡副文案写死"骨盆、臀部、腹股沟和髋关节"（symptom-stage :385/:431）；:608 隐藏整个动作区（含进入关键确认）；唯一出口"重写症状描述"藏在默认折叠的 `<details>` 里；下肢选择器由 `showIntakeQuestion("不舒服的位置")` 控制、与不支持提示无耦合。

修复（本批提交，沿用另三张 route-note 的既有模式）：

- **① 出口**：提示卡加两个显式按钮——「修改症状描述」（→rewriteIntakeDescription，回输入框）＋「保存本次信息」（→saveRecord("待医学评估")，与血管/神经/刺痛卡同款）。专业与自助两视图都改。
- **② 副文案动态化**：改为「现在只开放大腿至足部。{unsupportedDescriptionRegion}的问题不会被套进膝踝方案。可以修改描述换成支持的部位，也可以保存本次信息，先完成线下评估。」——说肩的用户看到肩，不再被塞骨盆髋。
- **③ 选择器不收起（有意取舍）**：描述同时含不支持区与支持区时（如"右臀部到小腿发麻"确有小腿成分），收起选择器会误伤；死胡同由出口解决。"支持区共存时能否继续单链"属 10 号方案多链调度范畴，登记为已知限制，不在本批扩面。

## 验证

- 真实浏览器（Playwright，全新 context 过三道 gate 真实键入）：P6"左肩膀抬手时疼"→提示卡动态副文案含"肩关节与肩胛"、错误硬编码文案消失、两出口齐、点修改描述回输入框；P7"右臀部到小腿发麻"→副文案含"髋关节与大腿"、保存生效（案例 HDZNJJ6S 已保存到本机）。12/12 PASS。
- 缺陷 1 为纯字面量替换（渲染路径未动，测试侧 R4 旧观察恰好证明这些字面量直达 UI）；git grep src 全域 30～60 清零、60～90 在位。
- 套件失败集合：stash 前后逐条相同（58 条，dev 过期 tests/ 副本预存红，零新增）；check:catalog（含新护栏）／typecheck／build 全绿；check:docs 仍 3 条已登记 B 类死链。
- B 类契约核对：测试树无 30～60/治疗卡指令/提示卡副文案字面钉（rendered-html:189 钉的是选择器 scope 行"暂未开放"，未动）；**本批零新增解钉项**。

## 请测试侧复核

R4（膝治疗卡 60～90）、R4'（任意肌肉候选"怎么做"兜底剂量）、P6/P7（出口可用性）纳入下轮走查；居家放松/双侧/训练前准备三处并入范围若有异议请提。
