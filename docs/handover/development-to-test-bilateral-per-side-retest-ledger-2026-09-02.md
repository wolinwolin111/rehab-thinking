...
## 4. 给测试侧的 SG-2 落点口径（已定，非待确认）

- **决策**：膝伸直前侧的逐侧复测台账按**专业模式（康复思路·给别人）**收口。理由：该处理是 released P0 单元，其证据门槛要求被动活动受限，而自助模式按 `p0AssessmentEvidenceForDecision` 强制剥离被动数据——自助拿不到此台账是**产品设计事实，不是缺陷**。owner 已就此授权按专业模式定稿。
- SG-2 断言请以专业模式落点为准；fixtureNote 已更新为实际渲染（左右两行 + 三按钮 + 确认双侧复测）。
- **自助逐侧复测覆盖 = 明确降级为独立条目，本批不做**：若将来要覆盖自助模式的逐侧复测，需换一个自助可产生处理单元的非 P0 靶点（如膝内侧松解 `knee-medial-*`），届时另开夹具与条目，不影响本 SG-2 收口。
