...
| 总结结果 tone（F-4/P4） | summary-stage rm-function-action-summary | is-retested 一律 mint（F-4：已复查≠改善） | 展示层 outcome 投影（improved/worse/unchanged/completed/neutral），只消费 retestLabel 生成时已用的同一组分数比较；mixed→neutral | 复测记录/义务（只读投影，无领域改动） | mobile-patient.css mint 一律规则改为 tone 分档 | **真实渲染通过（部分分支）**：三场景（improved/same/worse fixture）总结页 8 项动作全部 `is-outcome-completed`、em 色 rgb(89,101,121) 中性灰——修复前此处一律 mint 成功色，机制（类驱动 tone）已在活渲染路径证实。improved/worse 色档未见于此批 fixture（动作项均为"第一次能完成"无分数比较），逻辑层 7/7 覆盖；**improved/worse 视觉档留 RQ-4 尾项**。另发现**存量缺陷 F-6**：treatment-improved 场景入口双固定栏互相遮挡（continuationSuggestions 面板＋评估缺口面板同屏渲染，两个 position:fixed rail z-120 重叠），驱动只能 DOM-click 穿透——C 迁移将以互斥渲染＋dev 警告消除 |
