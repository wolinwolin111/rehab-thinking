...
> 分工（2026-09-08，第 32 轮 B 方案）：SYS-* 编号与守护映射的**权威清单**是 `tests/workflow/sys-invariant-traceability.json`（B 类，测试侧维护，含 33 个编号＝17 不变量＋16 场景）；本节承载**语义**。门禁硬检查＝守护文件存在＋anchor 真实，不再做文档↔清单交叉核对。
