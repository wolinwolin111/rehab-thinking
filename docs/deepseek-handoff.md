...
急性撞伤、明显肿胀、刺痛、麻电、进行性无力等安全场景应隐藏不安全的自主放松。
...
- 居家自主放松数据流（紧张检查 + 有效处理肌肉 + 训练主要肌肉，去重最多 2～3 个，安全场景隐藏，放在训练页与总结页居家训练模块）；
- 髌骨 `patella-mobility-unit` 处理/复测合并（一张处理卡列出受限方向、一张复测卡统一记录、按方向保存、恢复方向退出、纯被动复测模板）。
