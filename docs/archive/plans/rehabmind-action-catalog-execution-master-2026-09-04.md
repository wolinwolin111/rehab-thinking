...
**决策点**：
- D4-1 等长句式模板：「保持5秒，左右比较」在膝/踝/大腿各条统一成一个模板吗？（建议统一，逐条在审阅表给前后对照）**→ 已定：统一，逐条对照审（2026-09-04）。**
- D4-2 膝·腘绳肌自助句维持现 FRIENDLY 版「像要把脚跟向椅子下面拖，但不要真的移动」——确认不再改。**→ 已定：维持（§13.1）。**
## 7. 批次 5：处理候选族
...
**决策点**：
- D5-1 兜底卡「现在做」动作标签。**→ 已定：折中案（2026-09-04）**——标签直接用动作名；「怎么做」句（build-trial-targets-core:609）去掉重复的动作名改为「在不明显增加不适的范围内，缓慢完成 5～8 次。」；actionLabel 字段（:615）随迁。
...
**决策点**：
- D6-1 训练 purpose 当前单轨（两模式同文）。**→ 已定：单轨不拆；批次 6 顺带修 3 条流程词（2026-09-04）**——解剖学名词保留（臀中肌/骨盆/外翻等，owner 裁定），仅改：①ankle-achilles-eccentric-drop「证据最扎实的负荷动作」→「证据最扎实的加练动作」；②thigh-run-return「一次只加一项负荷」→「一次只加一样强度」；③calf-step-single-leg「送回高负荷场景」→「送回高强度场景」。其余三条含「代偿/协同」者（ankle-standing-hip-flexion、thigh-lateral-stability、thigh-lateral-step）owner 裁定**不变**。
...
| D4-1 | 等长句式统一模板 | 4 | **已定：统一，逐条对照审** |
| D4-2 | 腘绳肌自助句维持现文 | 4 | **已定：维持** |
| D5-1 | 兜底卡「现在做」用词 | 5 | **已定：折中案**（动作名＋怎么做去重） |
| D6-1 | 训练 purpose 单轨＋顺带修 3 条流程词 | 6 | **已定：单轨不拆；负荷×3 改、代偿/协同保留** |
...
## 13. C1/C2 续批裁定归档（2026-09-05，方案见 C1C2改造方案 Excel）

| # | 决策 | 结果 |
|---|---|---|
| C3/R | 9 组通用登记＋3 行旧标题清理 | **已做**（d27c4d7；R-1~3 死代码删除，目录优先早已接管） |
| C1-1 | 代偿 48 组迁目录 | **已做·缩减 20/48**（dd10806）。缩减原因：28 键的评估条目未入库（颈/肩/胸/肘/腕/腰/髋＋custom-action/ankle-knee-wall/thigh-bridge-check）——三层降级（目录→旧表→通用）保行为。⚠ 更正：本行原写"52 键新旧输出逐字相同"不成立——当时等价脚本从新位置读旧值属循环论证，且格式化脚本把 20 个数组压成空（代偿按钮实际落通用 4 条），该回归于 23c3089 按 4a82553 原始表逐字恢复并加 CAT-BAD-COMPENSATION-ID 校验修复 |
| C1-2 | 紧张位置 21 组迁目录 | **已裁：跳过**。前提崩塌——21 键全是运动知识 id，目录零对应（同 D4-1 零工作量模式）。未来部位入库时随条目顺带处理 |
| C2-1 | bilateralObserve 16 条并入评估条目 | **已做 15/16**（d010059）。knee-posterior-chain 未入库留旧表；helper 三层降级；消费方 2 处换 helper |
| C2-2 | 删 bilateralAssessmentCopy＋bilateralHow 逐条＋12 处接线 | **已裁：跳过**。实测全屏幕文本上该链只产生 3 处机械替换（膝提踵观察/走路做法/踝承重观察，均 患侧→左右两侧），非方案估的 16 条；且链是未入库部位兜底不可删。为 3 处改 12 处建队列代码风险收益不成立。未来部位入库后另批处理 |
| — | 全程验收 | 步骤 0–3 失败集合与基线逐条相同；编号分离后套件 53→54（新增 rendered-html:993 源码文字钉，行为未变，待测试侧解钉）；check:catalog/typecheck 全绿。归类接线见 §14 |

## 14. C1/C2 后续：编号分离与归类接线（2026-09-05，第 24–26 轮）

| 提交 | 内容 |
|---|---|
| 23c3089 | 代偿选项编号分离（编号存储、措辞显示、归类认编号）；修复步骤 1 空数组回归；CAT-BAD-COMPENSATION-ID 校验 |
| 381207a | 归类映射进词表（tags 字段＋compensationTagsFor 纯函数），删 workbench if 链；A 方案改指向死标签；CAT-DEAD-COMPENSATION-TAG 护栏（标签必须存在于候选全集） |
| b6f062f | 三组接线全部完成（11→23 编号有标签）；body-fwd-or-side-fall 拆分为 trunk-fwd-lean/trunk-side-lean；晃动家族跨池标签；push-off-opposite-leg 修正为 quadriceps（宿主在膝池） |

**关键设计约束（多部位扩展必读）**：候选打分只在当前部位池内做标签交集——归类标签必须落在选项宿主卡所属部位的候选池里，否则空转。本轮已逐条按此核实；未来同概念跨部位需按池配标签（词表 tags 可升级为按部位分组）。

**自查后两项裁定（owner 2026-09-05，均为有意接受，勿当 bug 修）**：
1. **拆分按钮的老记录高亮丢失＝接受**：上台阶卡 body-fwd-or-side-fall 拆为前倾/侧倒两钮后，拆分前勾过旧复合值的记录在该卡不再点亮对应格（数据完整、摘要文字在、归类仍按不命中）。因旧值无法区分前倾/侧倒，硬映射任一钮都是臆测，故不加兼容。
2. **全局词表的范围溢出＝接受**：文字→编号归一化作用于所有代偿值，使 3 张未入库但共用同文字的可达卡（ankle-knee-wall 足弓塌下→arch、thigh-bridge-check 骨盆一侧下沉→hip/glute、custom-action 措辞）一并获得正确归类/措辞。裁定：认"同一现象=同一编号=同一归类"的全局一致性优先于逐卡范围，保留该效果（肩/髋/腰等不可达卡同理，待其入库自然对齐）。

## 15. 过程教训（本轮自查新增）

1. **等价性验证禁止循环论证**：迁移类改动的"旧值"必须取自 git 基线（`git show <pre>:<file>`），不得从改动后的新位置读取——步骤 1 的空数组回归正因循环论证而漏检，直到编号分离时逐条比对原始表才暴露。
2. **禁止把 node_modules 以 junction 接入 git worktree**：`git worktree remove --force` 会沿 junction 递归清空真实依赖目录（本轮实际发生，靠 agent worktree 同锁文件副本恢复）。
3. **PowerShell 内联 node -e 含中文/正则/引号必坏**——一律写临时 .mjs 执行后删除（本轮多次违反此既有教训）。
4. **格式化脚本改数组字面量后必须逐条比对原始表**，不能只看 tsc 通过（tsc 不校验数组内容非空）。

## 16. 双轨剂量裁定与走查缺陷修复（2026-09-07，第 30 轮，提交 412c5db）

1. **临床裁定优先于搬运纪律**：f9721b6「松解类 30～60→60～90秒」当时只落进目录数据层，全部活渲染路径（膝 adapter、兜底剂量、内联候选、双侧、居家放松、训练前准备）未吃到裁定；批次 7 逐字镜像为零消费者死数据。owner 确认：逐字保真让位于剂量裁定，全部活路径统一 60～90（含死镜像同步）。
2. **兜底剂量删"必要时一次延长至90秒"**（owner 批准）：60～90 上限即 90，延长句自相矛盾；不发明新剂量（如 120），常规即封顶。
3. **并入范围批准**（owner）：双侧松解、居家放松（每处60～90秒，1～2轮）、训练前准备三处按松解类跟涨。
4. **不支持区死胡同修复**：提示卡副文案动态化＋双出口（修改描述/保存待线下）；**下肢选择器有意不收起**——混合描述（"臀部到小腿"）会被误伤，支持区共存时能否继续单链属 10 号方案多链调度，登记为已知限制。
5. 防漂移护栏 `CAT-RELEASE-DOSE-DRIFT`：src 任何 .ts/.tsx 出现「30～60秒」即构建失败（探针验证红→绿）。
6. 教训补一条：**裁定落地必须验证到活渲染路径**——数据层改完≠用户看到；golden 锁目录输出曾给出"已生效"的假象。
