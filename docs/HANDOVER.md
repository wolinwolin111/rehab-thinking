...
1. 双侧解析增强：「双/都…X侧更明显」→ 自动填 complaintPrioritySide（bilateral-flow-core 已有入参，只差 intake-complaint-core 提取规则）
2. 肌肉紧张度选择器的交互层级是否需要简化（当前：区域卡 → 展开侧别 → 或选接近出口）
3. 0 分改善解释文案的措辞是否需要按粉丝反馈调整
