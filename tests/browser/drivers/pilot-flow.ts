...
/**
 * 引导（自助康复）模式：真实产品页从急性踝描述出发，逐屏答到安全确认页并
 * 进入关键确认（停在 safetyStage 0）。D-2 骨性风险与急性踝安全停止两条测试
 * 共用此 proven 序列。描述刻意不含「肿」字，避免触发肿胀位置子屏。
 */
export async function driveGuidedAcuteAnkleToSafetyConfirmation(page: Page) {
  await openFreshProduct(page);
  await skipOnboarding(page);
  const input = await expectUniqueVisible(page, "症状输入框", page.locator("textarea:visible"));
