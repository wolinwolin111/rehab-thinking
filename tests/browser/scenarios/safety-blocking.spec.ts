...
test.fixme("D-3 前交叉韧带稳定性阳性：转医学评估出口 @scenario", async ({ page }) => {
  // 前置缺口：专项检查（special:knee-acl-stability）在专业膝部首轮队列中被
  // 排序预算挤出（详见 docs/quality/defect-special-queue-2026-08-30.md），
  // 浏览器层无法到达稳定性检查卡。修复后恢复完整链路：
  // 专业协助检查 + 专项检查能力 → 队列出现「前交叉韧带稳定性检查」→
  // 选「出现提示反应」→ T-04 确认 → 评估结果出现医学评估出口且无普通处理入口。
