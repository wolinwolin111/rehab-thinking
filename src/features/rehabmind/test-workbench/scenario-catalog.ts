...
export const PILOT_TEST_SCENARIOS: readonly PilotTestScenario[] = [
  ...
  {
    id: "recurrent-flare-acute",
    title: "反复踝急性再发骨性问答",
    description: "反复出现的踝问题、最近一次在今天内且有扭伤机制，关键确认应出现骨性风险问答。",
    mode: "page_boundary",
    target: "安全确认边界",
    initialProblem: "右踝反复疼，今天又崴了一下。",
    step: 1,
    snapshotOverrides: {
      intake: {
        regionId: "ankle-foot",
  ...
  {
    id: "recurrent-flare-chronic",
    title: "反复踝慢性再发无骨性问答",
    description: "反复出现的踝问题、最近一次在一周以上，即使有扭伤机制也不触发骨性风险问答。",
    mode: "page_boundary",
    target: "安全确认边界",
    initialProblem: "右踝反复疼，上次崴伤是一周以前。",
    step: 1,
    snapshotOverrides: {
      intake: {
        regionId: "ankle-foot",
  ...
  {
    id: "recurrent-flare-no-trauma",
    title: "反复踝自发再发无骨性问答",
    description: "反复出现的踝问题、最近一次在今天内但没有明确外伤机制，不触发骨性风险问答。",
    mode: "page_boundary",
    target: "安全确认边界",
    initialProblem: "右踝反复疼，今天早上突然疼起来，没有崴到。",
    step: 1,
    snapshotOverrides: {
      intake: {
        regionId: "ankle-foot",
  ...
  {
    id: "custom-action-assessment",
    title: "自定义动作复现卡",
    description: "自定义动作「跪坐」匹配不到任何标准功能项时，评估段出现 function:custom-action 复现卡；四档负荷选择（完全不承重/需承重可扶/只有原样负重/暂时不做）与急性肿胀硬闸「今天先不做现场复现」均可落点。",
    mode: "page_boundary",
    target: "评估边界",
    initialProblem: "右膝跪坐时前侧疼，想恢复正常跪坐。",
    step: 2,
    snapshotOverrides: {
      intake: {
        description: "右膝跪坐时前侧疼，希望恢复正常跪坐。",
        location: "膝关节前侧",
        customAction: "跪坐",
        reproduction: "跪坐",
        provocationTypes: ["运动过程中"],
      },
    },
    fixtureNote: "预期：评估段含「跪坐」标题的 function:custom-action 卡（非急性场景显示「能不能用不承重的方式模仿这个动作？」四档）；硬闸分支见 custom-action-deferred 场景。",
  },
  {
    id: "custom-action-deferred",
    title: "自定义动作硬闸推迟复现",
    description: "自定义动作卡命中急性/肿胀硬闸时显示「今天先不做现场复现」+ 记报评分 + 暂时不做，不渲染四档负荷选择。",
    mode: "page_boundary",
    target: "评估边界",
    initialProblem: "右膝跪坐时前侧疼，昨天又崴了一下，现在肿痛。",
    step: 2,
    snapshotOverrides: {
      intake: {
        description: "右膝跪坐时前侧疼，昨天又崴了一下，现在肿痛。",
        location: "膝关节前侧",
        customAction: "跪坐",
        reproduction: "跪坐",
        provocationTypes: ["运动过程中"],
    ...
    fixtureNote: "预期：function:custom-action 卡渲染「今天先不做这个动作的现场复现」提示 + 记报评分滑杆 + 「暂时不做，照常记录」按钮；不出现四档负荷选择。",
  },
  {
    id: "calf-local-assessment",
    title: "小腿局部评估卡（动作库新值上屏）",
    description: "小腿局部区评估队列：动作/力量/功能卡按目录取值渲染，验证批次 0-7 新值上屏与旧剂量字面零残留。",
    mode: "page_boundary",
    target: "评估边界",
    initialProblem: "右小腿后侧走路时酸胀，有两周了，想恢复正常走路。",
    step: 2,
    snapshotOverrides: {
      intake: {
        regionId: "calf-local",
    ...
    fixtureNote: "预期：小腿局部评估卡逐卡渲染（how/observe 来自动作目录）；旧剂量文案（完成5次/最多记录20个高质量次数/每组8～10个）零残留。",
  },
  {
    id: "ankle-assessment-cards",
    title: "踝评估卡（动作库新值上屏）",
    description: "踝足区评估队列：方向族/力量族/功能卡按目录取值渲染（含踮脚尖力量临床变更），验证新值上屏。",
    mode: "page_boundary",
    target: "评估边界",
    initialProblem: "右脚踝走路时外侧疼，有一周了，想恢复正常走路。",
    step: 2,
    snapshotOverrides: {
      intake: {
        regionId: "ankle-foot",
    ...
    fixtureNote: "预期：踝评估卡逐卡渲染；力量族标题为裁定新值（外翻力量（腓骨肌）/内翻力量（胫骨后肌）/踮脚尖力量）；「缩短脚掌」零残留。",
  },
  {
    id: "ankle-training-card",
    title: "踝训练卡（训练目录全量上屏）",
    description: "踝足区训练页：训练卡 how 来自训练目录（53 条全量），验证新值上屏与低负荷门。",
    mode: "page_boundary",
    target: "训练边界",
    initialProblem: "右脚踝走路时外侧疼，想恢复正常走路。",
    step: 4,
    snapshotOverrides: {
      intake: {
        regionId: "ankle-foot",
    ...
    fixtureNote: "预期：训练页渲染踝区训练卡（how 来自训练目录）；不出现旧剂量字面。",
  },
  {
