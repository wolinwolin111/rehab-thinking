...
export function SymptomStage(props: SymptomStageProps) {
  ...
  if (professionalIntake && intake.parsed) {
    ...
    return <section className="rm-page rm-professional-intake">
      ...
      {unsupportedDescriptionRegion ? <section className="rm-route-note is-waiting"><span>当前首发范围</span><h2>暂不支持{unsupportedDescriptionRegion}</h2><p>现在只开放大腿至足部。{unsupportedDescriptionRegion}的问题不会被套进膝踝方案。可以修改描述换成支持的部位，也可以保存本次信息，先完成线下评估。</p><div><button type="button" onClick={rewriteIntakeDescription}>修改症状描述</button><button type="button" onClick={() => saveRecord("待医学评估")}>保存本次信息</button></div></section> : null}
  ...
  return <section className="rm-page">
    ...
    {!intake.parsed ? <>
      ...
      {unsupportedDescriptionRegion ? <section className="rm-route-note is-waiting">
        ...
        <p>现在只开放大腿至足部。{unsupportedDescriptionRegion}的问题不会被套进膝踝方案。可以修改描述换成支持的部位，也可以保存本次信息，先完成线下评估。</p>
        <div><button type="button" onClick={rewriteIntakeDescription}>修改症状描述</button><button type="button" onClick={() => saveRecord("待医学评估")}>保存本次信息</button></div>
      </section> : null}
      {vascularDescriptionSignal ? <section className="rm-route-note is-waiting"><span>建议先线下确认</span><h2>描述中出现发凉或发白</h2><p>这可能与末端循环有关，不要先做强刺激处理。可以保存当前信息，优先完成线下评估。</p><button type="button" onClick={() => saveRecord("待医学评估")}>保存本次信息</button></section> : null}
      ...
      {showAllIntakeFields && !unsupportedDescriptionRegion && !selfNeuralReferral && !stabbingEarlyReferral && !vascularDescriptionSignal ? <div className="rm-page-actions rm-intake-actions" data-action-layout={keyConfirmationReady ? "split" : "single"}><span className={keyConfirmationReady ? "is-complete" : ""}>{keyConfirmationReady ? "症状信息已经够用了" : "还需补充："}{!keyConfirmationReady ? intakeMissingFields.slice(0, 6).map((label) => <button key={label} type="button" className="rm-missing-jump" onClick={() => document.getElementById(`field-${label}`)?.scrollIntoView({ behavior: "smooth", block: "center" })}>{label}</button>) : null}{!keyConfirmationReady && intakeMissingFields.length > 6 ? `等 ${intakeMissingFields.length} 项` : ""}</span>{keyConfirmationReady ? <button data-action-role="primary" type="button" className="rm-primary" onClick={enterKeyConfirmation}>进入关键确认</button> : null}</div> : null}
    </>}
  </section>;
}
