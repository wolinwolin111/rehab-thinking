...
export function SummaryStage({ view, actions }: { view: SummaryStageView; actions: SummaryStageActions }) {
  ...
  if (followupStage === "training") return <section className="rm-page">
    ...
    {tissuePathway.id === "standard" && followupTensionLocations.some((location) => !["没有明显差别", "两侧感觉接近", "暂不判断"].includes(location)) ? <section className="rm-training-preparation"><header><span>训练前准备</span><strong>先做一次轻柔松解</strong></header><div>{followupTensionLocations.filter((location) => !["没有明显差别", "两侧感觉接近", "暂不判断"].includes(location)).map((location) => <article key={location}><b>{location}</b><span>轻柔松解60～90秒</span></article>)}</div><footer>完成后直接开始训练，不新增复测步骤。</footer></section> : null}
    ...
    {followupWorsenedExercise ? <StatusNotice tone="danger" title={`${followupWorsenedExercise.title}后不适更重`}>先停止刚才的做法，看看停下来后是否缓解。<ActionButton variant="primary" onClick={() => beginAdverseReassessment({ source: "training", sourceId: followupWorsenedExercise.id, sourceLabel: followupWorsenedExercise.title, timing: "during", beforeScore: followupSessionScore, afterScore: followupSessionScore, relatedAssessmentIds: followupWorsenedExerciseAssessmentIds })}>处理这次加重</ActionButton></StatusNotice> : <>{exercises.length > 0 ? <StatusNotice tone="warning" live title="完成本次训练前，还需要记录每个动作的反馈"><span>未选择反馈的动作：{pendingFollowupFeedbackExercises.map((exercise) => exercise.title).join("、") || "无"}</span></StatusNotice> : null}<section className="rm-next-stage"><span>下次继续</span><h2>先复查以前的问题，再决定是否增加难度</h2></section>
    <ActionRail actions={[
  ...
  return <section className="rm-page">
    ...
    <section className="rm-followup-new"><div><span>安全复查 · 本次有没有新出现的麻、电感或放射痛？</span></div><div><button type="button" className={effectiveRedFlags.numbnessOrRadiation === "no" ? "is-selected" : ""} onClick={() => updateRedFlag("numbnessOrRadiation", "no")}>没有</button><button type="button" className={effectiveRedFlags.numbnessOrRadiation === "yes" ? "is-selected is-alert" : ""} onClick={() => updateRedFlag("numbnessOrRadiation", "yes")}>有</button></div></section>
    <section className="rm-followup-new"><div><span>安全复查 · 症状是否在加重（无力或麻木范围扩大）？</span></div><div><button type="button" className={effectiveRedFlags.progressiveWeakness === "no" ? "is-selected" : ""} onClick={() => updateRedFlag("progressiveWeakness", "no")}>没有</button><button type="button" className={effectiveRedFlags.progressiveWeakness === "yes" ? "is-selected is-alert" : ""} onClick={() => updateRedFlag("progressiveWeakness", "yes")}>有</button></div></section>
    {redFlagReview.needsReferral ? <section className="rm-route-note is-waiting"><span>建议先线下确认</span><h2>复查发现有神经相关或一直在加重的信号</h2><p>这种情况不适合自己练，也不建议自己松神经。可保存当前信息，由专业人员检查感觉范围和力量变化。</p><button type="button" onClick={() => saveRecord("待医学评估")}>保存本次信息</button></section> : null}
  ...
  const nextFocus = [
    ...
    ...Object.values(assessmentResults).some(stoppedFromFear) ? ["上次担心加重的动作，先从更低负荷试起"] : [],
