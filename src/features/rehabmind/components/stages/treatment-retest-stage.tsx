...
import { ChiefOutcomeSummary, StageOutcomeSections } from "@/src/features/rehabmind/components/stages/shared/stage-outcome-sections";
...
export function TreatmentRetestStage({ view, actions }: { view: TreatmentRetestStageView; actions: TreatmentRetestStageActions }) {
  const {
    ...
    assessments, bilateralPriorityResolution, localLimbDecision, tissuePathway,
    swellingGuidance, trialTargets, pendingFunctionRetestItems, activeTarget, activeCandidate, activeTargetSides,
    ...
    isThinkingMode, thinkingWorkbenchOpen, onThinkingWorkbenchOpenChange, retestLedgerItems, workbenchContext,
  } = view;
  const {
  ...
  function renderTreatmentWorkbench() {
    const stageStates = workbenchStageStates({
      canContinueSafety: workbenchContext.canContinueSafety,
      assessmentFlowComplete: workbenchContext.assessmentReadyForTreatment,
      completedAssessmentCount: workbenchContext.assessmentCompletedCount,
      totalAssessmentCount: workbenchContext.assessmentTotalCount,
      unresolvedProblemCount: unresolvedImmediateLedgerProblems.length,
      trialRecordCount: trialRecords.length,
      trainingComplete: workbenchContext.trainingComplete,
      trainingPlanSaved: workbenchContext.trainingPlanSaved,
      exerciseCount: workbenchContext.exerciseCount,
      isSummaryStep: false,
    });
    const stageItems = [
      { label: "症状与安全", state: stageStates[0], onClick: () => { onThinkingWorkbenchOpenChange(false); goToStep(1); } },
      { label: "评估", state: stageStates[1], onClick: () => { goToStep(2); } },
      { label: "问题台账", state: stageStates[2], onClick: () => { goToStep(2); } },
      { label: "处理与复测", state: stageStates[3], onClick: () => onThinkingWorkbenchOpenChange(false) },
      { label: "训练", state: stageStates[4], onClick: () => { if (treatmentComplete) { onThinkingWorkbenchOpenChange(false); goToStep(4); } } },
      { label: "总结", state: stageStates[5], onClick: () => { if (treatmentComplete) { onThinkingWorkbenchOpenChange(false); goToStep(5); } } },
    ];
    ...
    return <section className="rm-page rm-thinking-workbench">
      ...
      <section className="rm-workbench-columns">
        ...
        <article className="rm-workbench-module"><header><div><span>继续排查</span><strong>{continuationSuggestions.length}项</strong></div></header>{continuationSuggestions.length ? <><div className="rm-workbench-ledger"><section><b>还没有得到解释的方向</b><ul>{continuationSuggestions.map((suggestion) => <li key={suggestion.id}>{suggestion.title}</li>)}</ul></section></div><p className="rm-workbench-empty">完成面板中接受后会加入评估补查。</p></> : <p className="rm-workbench-empty">主诉已有解释时不追加检查。</p>}</article>
      </section>
      <ActionRail actions={[{ id: "continue-assessment", label: "继续评估", placement: "secondary", onClick: () => goToStep(2) }, { id: "back-treatment", label: "返回处理", placement: "primary", onClick: () => onThinkingWorkbenchOpenChange(false) }]} />
    </section>;
  }
  ...
  const activeFunctionEvidence = !activeFunctionObligations.length && isFunctionTarget && activeTarget
    ...
    : undefined;
  // completion-status 的疼痛基线：评估时因疼中断的那次打分；没有它维持“只问能否完成”。
  const flareBaselineScore = assessmentResults[activeTarget?.finding.id ?? ""]?.symptomScore;
  const flareNeedsScore = activeFunctionEvidence?.retestMode === "completion-status"
    && typeof flareBaselineScore === "number"
    && (functionRetestCompletion === "complete" || functionRetestCompletion === "unable" && functionRetestUnableReason === "pain");
  const treatmentRetestGate = resolveTreatmentRetestGate({
    ...
    hasBaselineScore: typeof flareBaselineScore === "number",
    score: postScore,
    baselineScore: flareBaselineScore,
  ...
  const updateFunctionRetest = (answerKey: string, patch: Partial<TreatmentFunctionRetestAnswer>) => {
    setTreatmentFunctionRetests((current) => ({
      ...current,
      [answerKey]: { ...(current[answerKey] ?? { completion: "" }), ...patch },
    }));
  };
  const functionRetestFields = activeFunctionObligations.length ? <section className="rm-function-retest-list" data-testid="function-retest-obligations">
    <header className="rm-retest-checklist-header"><div><span>功能动作复查</span><strong>{activeFunctionObligations.length}个动作</strong></div><small>活动范围和实际动作都记录完，才算完成本次复查。</small></header>
    {activeFunctionObligations.map((obligation) => <article key={obligation.assessmentId} data-assessment-id={obligation.assessmentId}>
        ...
        {(obligation.sides?.length ? obligation.sides : [undefined]).map((side) => {
          ...
          return <section key={answerKey} className="rm-function-retest-side" data-side={side}>
            {side ? <strong>{side}</strong> : null}
            <div className="rm-result-grid is-two">{([['complete', '能完成'], ['unable', '还是做不完']] as const).map(([value, label]) => <button type="button" key={value} className={answer.completion === value ? "is-selected" : ""} onClick={() => updateFunctionRetest(answerKey, { completion: value, unableReason: value === "complete" ? undefined : answer.unableReason, score: value === "unable" ? undefined : answer.score, scoreConfirmed: value === "unable" ? false : answer.scoreConfirmed })}>{label}</button>)}</div>
            {answer.completion === "unable" ? <div className="rm-function-retest-reason"><strong>主要是什么原因？</strong><div className="rm-result-grid is-two">{([['pain', '疼或不舒服'], ['weak', '没力或撑不住'], ['fear', '担心继续会加重']] as const).map(([value, label]) => <button type="button" key={value} className={answer.unableReason === value ? "is-selected" : ""} onClick={() => updateFunctionRetest(answerKey, { unableReason: value, ...(value !== "pain" ? { score: undefined, scoreConfirmed: false } : {}) })}>{label}</button>)}</div></div> : null}
            {obligation.mode === "ordinary" && answer.completion === "complete" ? <ScoreSlider compact value={answer.score ?? 0} selected={answer.scoreConfirmed ?? false} onChange={(score) => updateFunctionRetest(answerKey, { score, scoreConfirmed: true })} label={side ? `${side}现在有多不舒服？` : "现在有多不舒服？"} context={typeof obligation.baselineScore === "number" ? scoreBeforeContext(obligation.baselineScore) : undefined} /> : null}
            {obligation.mode === "completion-status" && typeof obligation.baselineScore === "number" && (answer.completion === "complete" || answer.completion === "unable" && answer.unableReason === "pain") ? <ScoreSlider compact value={answer.score ?? 0} selected={answer.scoreConfirmed ?? false} onChange={(score) => updateFunctionRetest(answerKey, { score, scoreConfirmed: true })} label={answer.completion === "complete" ? side ? `${side}完成时有多不舒服？` : "完成这个动作的时候，有多不舒服？" : side ? `${side}现在停下来时有多不舒服？` : "现在停下来时，有多不舒服？"} context={`处理前 ${obligation.baselineScore}/10 · 当时做不完时的疼`} /> : null}
          </section>;
        })}
        {obligation.mode === "completion-status" && typeof obligation.baselineScore !== "number" ? <div className="rm-retest-mode-note"><strong>{FUNCTION_COMPLETION_RETEST_COPY.title}</strong><span>{FUNCTION_COMPLETION_RETEST_COPY.description}</span></div> : null}
      </article>)}
  </section> : null;
  const retestReady = activeFunctionObligations.length ? functionRetestSummary.ready : functionRetestState.retestReady;
  ...
  const resultWithFunctionRetest = (baseResult: TrialResult) => combineRetestResults(
    baseResult,
    activeFunctionObligations.length ? functionRetestSummary.result : undefined,
  );
  ...
  const batchComplete = batchRangeComplete
    && chiefScoreComplete
    && (!activeFunctionObligations.length || functionRetestSummary.ready);
  ...
  const finalChiefRetestFragment = chiefNeedsFinalRetest && !treatmentFinalRetestConfirmed ? <>
    ...
    <section className="rm-treatment-final-retest">
      ...
      <ActionRail actions={[{ id: "record-final", label: "记录本轮最终结果", placement: "primary", disabled: !finalRetestComplete, onClick: () => { setTreatmentFinalRetestScore(hasFunctionChief ? postScore : directionChiefRetestScore); setTreatmentFinalRetestConfirmed(true); } }]} />
    </section>
  </> : null;
  const pendingFunctionRetestPanel = pendingFunctionRetestItems.length ? <section className="rm-complete-panel is-caution" data-testid="pending-function-retests">
    <span>本次复查还没完成</span>
    <h2>还有动作需要再试一次</h2>
    ...
    <ActionRail actions={[
      { id: "record-actions", label: "记录这些动作", placement: "primary", disabled: !functionRetestSummary.ready, onClick: finishOutstandingFunctionRetests },
  ...
  if (pendingKneeAssessmentCheck && !trialTargets.length) {
    ...
    return <section className="rm-page">
      ...
      <section className="rm-complete-panel is-caution">
        ...
        <ActionRail actions={[
          { id: "review", label: "查看评估记录", placement: "secondary", onClick: () => reviewCompletedStep(2) },
          { id: "supplement", label: "返回补充检查", placement: "primary", onClick: () => canOpenRelatedAssessment
            ? openAssessmentItem(relatedAssessmentId, "请补充这项检查；完成后再安排处理。")
            : reopenAssessment("还有一项膝关节检查需要补充；完成后再安排处理。") },
        ]} />
      </section>
    </section>;
  }
  if (treatmentWorsened) return <section className="rm-page">
    ...
    <section className="rm-complete-panel is-referral"><span>刚才的反应</span><h2>{mixedTreatmentOutcome ? "疼痛评分下降，但活动表现变差" : "症状或活动表现变差"}</h2><p>{mixedTreatmentOutcome ? "疼痛分数的下降和活动表现的变差需要分开记录。当前处理先停止，不把疼痛改善当作整体安全；接下来只确认变差的活动和相关检查。" : "先停止刚才的处理。接下来只确认症状变化和直接相关的检查，不会返回整套评估。"}</p><ActionRail actions={[
            { id: "confirm-worse", label: "确认加重后的变化", placement: "primary", onClick: () => beginAdverseReassessment({ source: "treatment", sourceId: lastWorsenedTreatment?.candidateId ?? "treatment", sourceLabel: lastWorsenedTreatment?.treatmentName ?? lastWorsenedTreatment?.candidateTitle ?? "刚才的处理", timing: "immediate", beforeScore: lastWorsenedTreatment?.beforeScore ?? intake.baselineScore, afterScore: lastWorsenedTreatment?.afterScore ?? lastChiefScore, relatedAssessmentIds: worsenedRelatedAssessments }) },
            { id: "supplement", label: "补充症状信息", placement: "secondary", onClick: () => goToStep(0) },
            { id: "save", label: "保存并结束", placement: "tertiary", onClick: () => saveRecord("处理后加重，待重新评估") },
          ]} /></section>
  </section>;
  if (bilateralCheckpointRequired) {
    ...
    return <section className="rm-page rm-bilateral-checkpoint">
      ...
      <section className="rm-complete-panel">
        ...
        {(() => {
          // 表现层动作映射（评审单 R-02 §6.4 → P2 ActionRail 迁移）：底栏最多
          // 三个按钮，多余出口降到正文"其他安全选择"，不再让四个条件按钮竞争
          // 三个角色槽。布局与皮肤由 ActionRail 所有（plan §7.7）。
          type CheckpointAction = { key: string; label: string; role: "primary" | "secondary" | "tertiary"; onClick: () => void };
          const all: CheckpointAction[] = [];
          ...
          const railActions = [all.find((a) => a.role === "secondary"), all.find((a) => a.role === "primary"), all.find((a) => a.role === "tertiary")].filter(Boolean) as CheckpointAction[];
          const inline = all.filter((a) => !railActions.includes(a));
          return <>
            <ActionRail ariaLabel="当前步骤操作" actions={railActions.map((action) => ({ id: action.key, label: action.label, placement: action.role, onClick: action.onClick }))} />
  ...
  if (bilateralNeedsReferral) return <section className="rm-page">
    <StepHeading eyebrow="第4步 · 处理与即时复测" title="本次处理已暂停" />
    <section className="rm-complete-panel is-referral"><span>双侧反馈</span><h2>处理后症状加重</h2><p>先停止刚才的处理。返回检查重新确认，或者保存记录后由专业人员评估。</p><ActionRail actions={[{ id: "reopen", label: "返回相关检查", placement: "primary", onClick: () => reopenAssessment() }, { id: "save-end", label: "保存并结束", placement: "secondary", onClick: () => saveRecord("待医学评估") }]} /></section>
  </section>;
  ...
  if (!trialTargets.length) {
    ...
    if (pendingFunctionRetestPanel) {
      return <section className="rm-page">
        <StepHeading eyebrow="第4步 · 处理与即时复测" title="完成本次复查" />
        {pendingFunctionRetestPanel}
      </section>;
    }
    // 队列空且还需要最终主诉复测时，先做统一复测，再进入未达标问题的成果面板。
    ...
    if (intake.side === "双侧/中间" && !midpointDecisionDone) {
      ...
      return <section className="rm-page rm-bilateral-checkpoint">
        ...
        <section className="rm-complete-panel">
          ...
          {(() => {
            // 表现层动作映射（R-02 状态 B/C → P2 ActionRail 迁移）：底栏最多
            // 三个、恰好一个 primary，超出容量降到正文"其他安全选择"。
            type CheckpointAction = { key: string; label: string; role: "primary" | "secondary" | "tertiary"; onClick: () => void };
            const all: CheckpointAction[] = [];
    ...
    if (hasUnresolvedImmediateTreatmentProblem && !continuationExitActive) {
      return <section className="rm-page">
        ...
        <section className="rm-complete-panel is-caution">
          <span>本轮处理已完成</span>
          <ChiefOutcomeSummary chiefScoreComparable={chiefScoreComparable} chiefImproved={chiefImprovedDuringTreatment} chiefRetested={chiefRetestCompletedDuringTreatment} noImmediateTreatmentResponse={noImmediateTreatmentResponse} baselineScore={intake.baselineScore} lastChiefScore={lastChiefScore} reportedActions={reportedActionSummary(intake)} hasClearChiefAction={hasClearChiefAction(intake)} noActionCopy="这次没有固定的加重动作，已记录其他活动和症状变化。" />
          ...
          <ActionRail actions={[
            { id: "review", label: "查看评估记录", placement: "secondary", onClick: () => reviewCompletedStep(2) },
            { id: "reconfirm", label: hasSpecificAssessmentGap ? assessmentGapActionLabel(assessmentGap) : "重新确认评估答案", placement: "tertiary", onClick: hasSpecificAssessmentGap ? () => openAssessmentItem(assessmentGap!.assessmentId, "请完成这项检查，完成后再安排处理。") : editCompletedAssessment },
            { id: "to-training", label: "进入训练", placement: "primary", onClick: () => goToStep(4) },
          ]} />
        </section>
      </section>;
    }
    ...
    const completedAttemptDetail = rangeChangedWithoutChiefChange
      ...
      : treatmentCoverage.decision === "stop-covered-no-effect" || noImmediateTreatmentResponse
        ? "相关处理已经完成。今天先保留低强度的活动；症状持续不变、变重或影响走路时，建议线下评估。"
    ...
    return <section className="rm-page">
      ...
      <section className={`rm-complete-panel ${assessmentEvidenceInsufficient ? "is-caution" : ""}`}>
        ...
        <p>{assessmentEvidenceInsufficient
          ? "刚才的主要检查都没有得到可判断结果。这不代表没有问题，也不能凭这个决定做肌肉处理还是训练。"
          : completedTreatmentAttempt
        ...
        {treatmentComplete && continuationSuggestions.length ? (
          <section className="rm-outcome-unexplained">
            ...
            <ActionRail note="也可以先进入训练或保存记录；这些方向以后仍然可以补查。" actions={[{ id: "accept-suggestions", label: "继续检查这些方向", placement: "primary", onClick: () => acceptContinuationSuggestions(continuationSuggestions.map((item) => item.id)) }]} />
          </section>
        ) : null}
        {assessmentEvidenceInsufficient
          ? <ActionRail actions={[
            { id: "gap", label: assessmentGapActionLabel(assessmentGap), placement: "primary", onClick: () => assessmentGap ? openAssessmentItem(assessmentGap.assessmentId, "已定位到需要补充的检查；完成后会回到原流程。") : reopenAssessment() },
            { id: "supplement", label: intakeMissingFields[0] ? `补充${intakeMissingFields[0]}` : "补充症状信息", placement: "secondary", onClick: () => { const missing = intakeMissingFields[0]; if (missing) { setStep(0); setReviewStep(null); jumpToIntakeQuestion(missing); } else reopenAssessment(); } },
            { id: "save-later", label: "保存，之后继续", placement: "tertiary", onClick: () => saveRecord("评估未完成") },
          ]} />
          : completedTreatmentAttempt
            ? <ActionRail actions={[
            { id: "review", label: "查看评估记录", placement: "secondary", onClick: () => reviewCompletedStep(2) },
            { id: "forward", label: noImmediateTreatmentResponse ? "查看低强度活动" : "查看训练与居家方案", placement: "primary", onClick: () => goToStep(4) },
            { id: "save", label: "保存并结束", placement: "tertiary", onClick: () => saveRecord(noImmediateTreatmentResponse ? "处理后主诉未明显改善" : "处理完成") },
          ]} />
          : unresolvedLedgerProblem && hasSpecificAssessmentGap
            ? <ActionRail actions={[
            { id: "review", label: "查看评估记录", placement: "secondary", onClick: () => reviewCompletedStep(2) },
            { id: "gap", label: assessmentGapActionLabel(assessmentGap), placement: "primary", onClick: () => openAssessmentItem(assessmentGap!.assessmentId, "请完成这项检查；完成后会重新安排后续内容。") },
          ]} />
            : unresolvedLedgerProblem
              ? <ActionRail actions={[
            { id: "review", label: "查看评估记录", placement: "secondary", onClick: () => reviewCompletedStep(2) },
            { id: "supplement", label: "补充症状信息", placement: "primary", onClick: () => goToStep(0) },
            { id: "save", label: "保存并结束", placement: "tertiary", onClick: () => saveRecord("现有检查未形成明确处理方向") },
          ]} />
            : <ActionRail actions={[
            { id: "review", label: "查看评估记录", placement: "secondary", onClick: () => reviewCompletedStep(2) },
            { id: "edit", label: "修改评估答案", placement: "tertiary", onClick: editCompletedAssessment },
            { id: "forward", label: treatmentEmptyState.action, placement: "primary", onClick: () => goToStep(4) },
          ]} />}
      </section>
    </section>;
  }
  return <section className="rm-page" data-rehabmind-test="treatment-page" data-planned-retest-action-key={canonicalRetestAction(plannedRetestLabel)}>
    <StepHeading eyebrow="第4步 · 处理与即时复测" title="针对性处理" />
    {isThinkingMode ? <button type="button" className="rm-workbench-back" onClick={() => onThinkingWorkbenchOpenChange(true)}>阶段工作台</button> : null}
    ...
    {!treatmentComplete && activeTarget && activeCandidate ? <>
      ...
      {!showingRetest && canShowOptionalProfessionalTreatment && activeTarget.optionalCandidates?.length ? <details className="rm-optional-treatment"><summary>可选处理（{activeTarget.optionalCandidates.length}）</summary><p>核心处理后仍有问题时，再从这里补充。</p><div>{activeTarget.optionalCandidates.map((candidate) => { const selectionKey = optionalTreatmentSelectionKey(activeTarget.id, candidate.id); const added = selectedOptionalCandidateIds.includes(selectionKey); return <button type="button" key={candidate.id} disabled={added} onClick={() => setSelectedOptionalCandidateIds((current) => [...current, selectionKey])}><strong>{candidateTreatmentName(candidate)}</strong><span>{added ? "已加入" : "加入本次处理"}</span></button>; })}</div></details> : null}
       {isTimeBasedTarget ? <ActionRail actions={[{ id: "finish-time", label: "完成这项处理", placement: "primary", onClick: () => finishTrial("partial", true) }]} /> : canReuseLatestRetest ? <ActionRail actions={[{ id: "reuse-next", label: "继续下一项", placement: "primary", onClick: continueWithReusedRetest }]} /> : isUnspecifiedChiefTarget ? <section className="rm-retest rm-no-action-retest">
        <header><span>本次不做动作评分</span><h2>目前没有确认会引起不适的动作</h2><p>先保存这项处理。活动受限会按该方向的比较方式单独复测，肿胀和压痛留到后续复查。</p></header>
        <ActionRail actions={[{ id: "finish-noaction", label: "完成并继续", placement: "primary", onClick: () => finishTrial("partial", false, undefined, true) }]} />
       </section> : !showingRetest && activeTarget.id === "target:chief" && (chiefImprovedDuringTreatment || chiefRetestCompletedDuringTreatment) && !isBatchRangeTarget && !activeFunctionObligations.length ? <ActionRail actions={[{ id: "finish-same", label: "完成这项处理，继续下一项", placement: "primary", onClick: () => finishTrial("same", false, undefined, true) }]} /> : !showingRetest ? <ActionRail actions={[{ id: "finish-retest", label: isResidualReviewStep ? "开始复查" : noRetestNeededAfterLatestResult ? "完成并继续下一项" : activeTargetIsBilateral && activeTargetPendingSides.length ? `完成${activeTargetCurrentSide ?? "当前侧"}，继续另一侧` : activeCandidateGroup.length > 1 ? "本轮处理完成，统一复测" : isRangeTarget ? singleRangeRetestsChief ? "处理完成，复测主诉和活动范围" : "处理完成，复测活动范围" : isStrengthSymptomTarget ? "处理完成，复测发力" : hasClearChiefAction(intake) ? "处理完成，复测原来的动作" : "处理完成，复测这个动作", placement: "primary", onClick: handleTreatmentCompletion }]} /> : intake.side === "双侧/中间" && activeTargetSides.length && !(isBatchRangeTarget && activeFunctionObligations.length) ? <section className="rm-retest rm-bilateral-retest" data-testid="bilateral-retest-ledger" data-priority-side={intake.prioritySide}><header><span>双侧分别复测</span><h2>分别记录左右两侧处理后的变化</h2><small>同一处理卡只展示一次，但左右结果不能互相覆盖。</small></header><div className="rm-bilateral-side-retest-list">{activeTargetSides.map((side) => { const value = bilateralRetestResponses[side]; const sideKey = side === "左侧" ? "left" : "right"; return <article key={side} data-testid={`bilateral-retest-${sideKey}`} data-status={value ?? "pending"}><strong>{side}</strong><div className="rm-result-grid is-three">{([['better','轻了'],['same','没变化'],['worse','更重']] as const).map(([response, label]) => <button type="button" key={response} data-testid={`bilateral-retest-${sideKey}-${response}`} aria-pressed={value === response} className={value === response ? "is-selected" : ""} onClick={() => setBilateralRetestResponses((current) => ({ ...current, [side]: response }))}>{label}</button>)}</div></article>; })}</div>{functionRetestFields}<button type="button" className="rm-primary" data-testid="bilateral-retest-confirm" disabled={activeTargetSides.some((side) => !bilateralRetestResponses[side]) || activeFunctionObligations.length > 0 && !functionRetestSummary.ready} onClick={() => { const responses = activeTargetSides.map((side) => bilateralRetestResponses[side]); const bilateralResult = responses.includes("worse") ? "worse" : responses.includes("better") ? "better" : "same"; const result = resultWithFunctionRetest(bilateralResult); setBilateralNeedsReferral(result === "worse"); finishTrial(result); }}>确认双侧复测</button><p>任一侧加重都停止后续同类处理；只有一侧有改善时，另一侧仍保留为未改善。</p></section> : intake.side === "双侧/中间" && !(isBatchRangeTarget && activeFunctionObligations.length) ? <section className="rm-retest rm-bilateral-retest"><header><span>整体反馈</span><h2>和刚才比，双侧的疼痛或轻松感有变化吗？</h2></header><div className="rm-result-grid">{([['better','轻了'],['same','没变化'],['worse','更重']] as const).map(([value,label]) => <button type="button" key={value} disabled={activeFunctionObligations.length > 0 && !functionRetestSummary.ready} onClick={() => { const result = resultWithFunctionRetest(value); setBilateralNeedsReferral(result === "worse"); finishTrial(result); }}>{label}</button>)}</div>{functionRetestFields}<p>当前处理没有明确侧别，先记录整体反应。</p></section> : isBatchRangeTarget ? <section className={`rm-retest rm-batch-range-retest rm-followup-retest ${isPatellaCombinedUnit ? "is-combined-patella" : ""}`}>
         {isPatellaCombinedUnit ? <header className="rm-combined-retest-header"><span>同一个动作 · 做完马上再试一次</span><h2>刚才受限的髌骨方向</h2><small>只复测刚才标记受限的方向，并记录活动范围和不适。</small></header> : null}
        ...
        <section className={`rm-auto-result is-${movementResponse === "both-match" ? "better" : ["passive-match-active-limited", "better-passive-limited", "passive-limited"].includes(movementResponse) ? "partial" : movementResponse || "waiting"}`}><span>复测结果</span><strong>{movementResponse === "both-match"
          ...
          : movementResponse === "passive-match-active-limited"
            ...
            : movementResponse === "worse"
              ? "活动范围比处理前更小或更痛"
              : `请选择处理后与${activeComparisonTarget}相比的活动范围`}</strong><button type="button" className="rm-primary" disabled={!movementResponse || (singleRangeRetestsChief && !postScoreConfirmed) || !(singleRangeRetestsChief ? singleRangeDiscomfort : postDiscomfort) || ((singleRangeRetestsChief ? singleRangeDiscomfort : postDiscomfort) === "yes" && !(singleRangeRetestsChief ? singleRangeScoreConfirmed : postScoreConfirmed)) || activeFunctionObligations.length > 0 && !functionRetestSummary.ready} onClick={() => {
                if (movementResponse === "both-match") finishTrial(resultWithFunctionRetest("better"));
                else {
                  const nextCandidateType = nextRangeCandidateType(movementResponse, activeRangeAllowsPassive && canMobilizeJoint);
                  if (nextCandidateType) finishTrial(resultWithFunctionRetest("partial"), false, nextCandidateType);
                  else finishTrial(resultWithFunctionRetest("worse"));
                }
              }}>继续</button></section>
      </section> : <section className="rm-retest">
        <header><span>复测动作</span><h2>{retestActionTitle}</h2></header>
        {activeFunctionObligations.length ? functionRetestFields : isFunctionTarget ? <section className="rm-motion-answer-block">
          <h3>现在这个动作能完成了吗？</h3>
          <p className="rm-choice-hint">以「动作能从头做到尾」为准，姿势不标准、有借力也算完成。</p>
          <div className="rm-result-grid is-two">{([["complete", "能完成"], ["unable", "还是做不完"]] as const).map(([value, label]) => <button type="button" key={value} className={functionRetestCompletion === value ? "is-selected" : ""} onClick={() => { setFunctionRetestCompletion(value); if (value === "complete") setFunctionRetestUnableReason(""); }}>{label}</button>)}</div>
          {functionRetestCompletion === "unable" ? <section className="rm-motion-answer-block is-followup">
    ...
    </> : pendingFunctionRetestPanel ?? (finalChiefRetestFragment ? finalChiefRetestFragment : treatmentWorsened ? <section className="rm-complete-panel is-referral"><span>处理已停止</span><h2>刚才的处理使症状或活动表现加重</h2><p>不要继续叠加处理或增加训练难度。请重新确认刚才加重的动作和位置；无法判断时保存记录并请专业人员协助。</p><ActionRail actions={[
          { id: "reopen", label: "重新评估", placement: "primary", onClick: () => reopenAssessment("已返回本次评估；请重新确认刚才加重的动作和症状。") },
          { id: "supplement", label: "补充症状信息", placement: "secondary", onClick: () => goToStep(0) },
          { id: "save", label: "保存并结束", placement: "tertiary", onClick: () => saveRecord("处理后加重，待重新评估") },
        ]} /></section> : bilateralNeedsReferral ? <section className="rm-complete-panel is-referral"><span>处理复测结束</span><h2>两侧处理后症状加重</h2><p>先停止本轮处理，建议由专业人员重新评估，再决定是否继续训练。</p><ActionRail actions={[{ id: "reopen", label: "重新评估", placement: "secondary", onClick: () => reopenAssessment() }, { id: "save-medical", label: "保存并结束本次", placement: "primary", onClick: () => saveRecord("待医学评估") }]} /></section> : persistentStabbing ? <section className="rm-complete-panel is-referral"><span>处理复测结束</span><h2>刺痛仍然存在</h2><div className="rm-final-score"><b>{intake.baselineScore}</b><i>→</i><strong>{lastChiefScore}</strong><small>已保留有效处理方向</small></div><p>相关的自助处理已经完成。原动作仍会刺痛，建议先做线下专业评估，再决定后续训练。</p><ActionRail actions={[{ id: "reopen", label: "重新评估", placement: "secondary", onClick: () => reopenAssessment() }, { id: "save-medical", label: "保存并结束本次", placement: "primary", onClick: () => saveRecord("待医学评估") }]} /></section> : <section className={`rm-complete-panel ${noImmediateTreatmentResponse ? "is-caution" : ""}`}><span>本阶段成果</span><ChiefOutcomeSummary chiefScoreComparable={chiefScoreComparable} chiefImproved={chiefImprovedDuringTreatment} chiefRetested={chiefRetestCompletedDuringTreatment} noImmediateTreatmentResponse={noImmediateTreatmentResponse} baselineScore={intake.baselineScore} lastChiefScore={lastChiefScore} reportedActions={reportedActionSummary(intake)} hasClearChiefAction={hasClearChiefAction(intake)} bilateralNoScore={intake.side === "双侧/中间"} noActionCopy="已记录本次活动和症状变化。" /><StageOutcomeSections effectiveFocusLabels={effectiveFocusLabels} effectiveControlLabels={effectiveControlLabels} recoveredRangeLabels={recoveredRangeLabels} improvedRangeLabels={improvedRangeLabels} trackObservationLabels={trackObservationLabels} strengthProblemTitles={weakStrengthProblems.map((finding) => finding.title)} />{noImmediateTreatmentResponse ? <section className="rm-no-response-note"><strong>本次试处理没有改变不适</strong><p>先不要增加训练难度；今天只保留轻柔的基础活动。症状持续不变、变重或影响承重时，建议线下重新评估。</p></section> : null}<ActionRail actions={[{ id: "review", label: "查看评估记录", placement: "secondary", onClick: () => reviewCompletedStep(2) }, { id: "forward", label: noImmediateTreatmentResponse ? "查看轻柔的基础活动" : "查看训练与居家方案", placement: "primary", onClick: () => goToStep(4) }]} /></section>) }
  </section>;
}
