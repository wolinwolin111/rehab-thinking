...
export function TreatmentRetestStage({ view, actions }: { view: TreatmentRetestStageView; actions: TreatmentRetestStageActions }) {
  ...
  function renderTreatmentWorkbench() {
    ...
    return <section className="rm-page rm-thinking-workbench">
      ...
      <div className="rm-page-actions split" data-action-layout="split"><button data-action-role="secondary" type="button" onClick={() => goToStep(2)}>继续评估</button><button data-action-role="primary" type="button" className="rm-primary" onClick={() => onThinkingWorkbenchOpenChange(false)}>返回处理</button></div>
    </section>;
  }
  ...
  const finalChiefRetestFragment = chiefNeedsFinalRetest && !treatmentFinalRetestConfirmed ? <>
    ...
    <section className="rm-treatment-final-retest">
      ...
      <div className="rm-one-action" data-action-layout="single"><button data-action-role="primary" type="button" className="rm-primary" disabled={!finalRetestComplete} onClick={() => { setTreatmentFinalRetestScore(hasFunctionChief ? postScore : directionChiefRetestScore); setTreatmentFinalRetestConfirmed(true); }}>记录本轮最终结果</button></div>
    </section>
  </> : null;
  const pendingFunctionRetestPanel = pendingFunctionRetestItems.length ? <section className="rm-complete-panel is-caution" data-testid="pending-function-retests">
    ...
    <div className="rm-page-actions split" data-action-layout="split">
      <button data-action-role="primary" type="button" className="rm-primary" disabled={!functionRetestSummary.ready} onClick={finishOutstandingFunctionRetests}>记录这些动作</button>
      <button data-action-role="secondary" type="button" onClick={() => saveRecord("康复中")}>保存，之后继续</button>
    </div>
  </section> : null;
  ...
  if (pendingKneeAssessmentCheck && !trialTargets.length) {
    ...
    return <section className="rm-page">
      ...
      <section className="rm-complete-panel is-caution">
        ...
        <div className="rm-page-actions split" data-action-layout="split">
          <button data-action-role="secondary" type="button" onClick={() => reviewCompletedStep(2)}>查看评估记录</button>
          <button data-action-role="primary" type="button" className="rm-primary" onClick={() => canOpenRelatedAssessment
  ...
  if (bilateralNeedsReferral) return <section className="rm-page">
    <StepHeading eyebrow="第4步 · 处理与即时复测" title="本次处理已暂停" />
    <section className="rm-complete-panel is-referral"><span>双侧反馈</span><h2>处理后症状加重</h2><p>先停止刚才的处理。返回检查重新确认，或者保存记录后由专业人员评估。</p><div className="rm-page-actions split" data-action-layout="split"><button data-action-role="primary" type="button" className="rm-primary" onClick={() => reopenAssessment()}>返回相关检查</button><button data-action-role="secondary" type="button" onClick={() => saveRecord("待医学评估")}>保存并结束</button></div></section>
  </section>;
      ...
      return <section className="rm-page">
        ...
        <section className="rm-complete-panel is-caution">
          ...
          <div className="rm-page-actions three" data-action-layout="three">
            <button data-action-role="secondary" type="button" onClick={() => reviewCompletedStep(2)}>查看评估记录</button>
            <button data-action-role="tertiary" type="button" onClick={hasSpecificAssessmentGap ? () => openAssessmentItem(assessmentGap!.assessmentId, "请完成这项检查，完成后再安排处理。") : editCompletedAssessment}>{hasSpecificAssessmentGap ? assessmentGapActionLabel(assessmentGap) : "重新确认评估答案"}</button>
            <button data-action-role="primary" type="button" className="rm-primary" onClick={() => goToStep(4)}>进入训练</button>
          </div>
        </section>
      </section>;
    }
    ...
    return <section className="rm-page">
      ...
      <section className={`rm-complete-panel ${assessmentEvidenceInsufficient ? "is-caution" : ""}`}>
        ...
        {assessmentEvidenceInsufficient
          ? <div className="rm-page-actions three" data-action-layout="three"><button data-action-role="primary" type="button" className="rm-primary" onClick={() => assessmentGap ? openAssessmentItem(assessmentGap.assessmentId, "已定位到需要补充的检查；完成后会回到原流程。") : reopenAssessment()}>{assessmentGapActionLabel(assessmentGap)}</button><button data-action-role="secondary" type="button" onClick={() => { const missing = intakeMissingFields[0]; if (missing) { setStep(0); setReviewStep(null); jumpToIntakeQuestion(missing); } else reopenAssessment(); }}>{intakeMissingFields[0] ? `补充${intakeMissingFields[0]}` : "补充症状信息"}</button><button data-action-role="tertiary" type="button" onClick={() => saveRecord("评估未完成")}>保存，之后继续</button></div>
          : completedTreatmentAttempt
            ? <div className="rm-page-actions three" data-action-layout="three"><button data-action-role="secondary" type="button" onClick={() => reviewCompletedStep(2)}>查看评估记录</button><button data-action-role="primary" type="button" className="rm-primary" onClick={() => goToStep(4)}>{noImmediateTreatmentResponse ? "查看低强度活动" : "查看训练与居家方案"}</button><button data-action-role="tertiary" type="button" onClick={() => saveRecord(noImmediateTreatmentResponse ? "处理后主诉未明显改善" : "处理完成")}>保存并结束</button></div>
          : unresolvedLedgerProblem && hasSpecificAssessmentGap
            ? <div className="rm-page-actions split" data-action-layout="split"><button data-action-role="secondary" type="button" onClick={() => reviewCompletedStep(2)}>查看评估记录</button><button data-action-role="primary" type="button" className="rm-primary" onClick={() => openAssessmentItem(assessmentGap!.assessmentId, "请完成这项检查；完成后会重新安排后续内容。")}>{assessmentGapActionLabel(assessmentGap)}</button></div>
            : unresolvedLedgerProblem
              ? <div className="rm-page-actions three" data-action-layout="three"><button data-action-role="secondary" type="button" onClick={() => reviewCompletedStep(2)}>查看评估记录</button><button data-action-role="primary" type="button" className="rm-primary" onClick={() => goToStep(0)}>补充症状信息</button><button data-action-role="tertiary" type="button" onClick={() => saveRecord("现有检查未形成明确处理方向")}>保存并结束</button></div>
            : <div className="rm-page-actions three" data-action-layout="three"><button data-action-role="secondary" type="button" onClick={() => reviewCompletedStep(2)}>查看评估记录</button><button data-action-role="tertiary" type="button" onClick={editCompletedAssessment}>修改评估答案</button><button data-action-role="primary" type="button" className="rm-primary" onClick={() => goToStep(4)}>{treatmentEmptyState.action}</button></div>}
      </section>
    </section>;
  }
