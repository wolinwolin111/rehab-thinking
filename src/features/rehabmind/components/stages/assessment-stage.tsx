...
export function AssessmentStage(props: AssessmentStageProps) {
  ...
  return <section className="rm-page rm-thinking-workbench">
    ...
    <ActionRail actions={[
      { id: "back-safety", label: "返回安全确认", placement: "secondary", onClick: () => goToStep(1) },
      { id: "to-treatment", label: "评估完成，进入处理", placement: "primary", disabled: !assessmentReadyForTreatment, onClick: () => { setTransitionTarget("treatment"); setThinkingWorkbenchOpen(false); } },
    ]} />
  </section>;
  }
  ...
  if (adverseResponse && (!adverseCaptureComplete(adverseResponse) || !adverseConfirmedAssessmentIds.includes("__capture__"))) {
    ...
    return <section className="rm-page rm-adverse-page">
      ...
      <ActionRail actions={[
        { id: "save-later", label: "保存，稍后继续", placement: "secondary", onClick: () => saveRecord("待复查") },
        { id: "confirm-continue", label: "确认并继续", placement: "primary", disabled: !adverseCaptureComplete(adverseResponse), onClick: () => setAdverseConfirmedAssessmentIds(["__capture__"]) },
      ]} />
    </section>;
  }
  ...
  if (adverseResponse && adverseResolution === "regress-training") return <section className="rm-page rm-adverse-page">
    ...
    <ActionRail actions={[
      { id: "still-worse", label: "退阶后仍然加重", placement: "secondary", onClick: () => setAdverseResponse((current) => current ? { ...current, regressionAttempted: true, settledAfterStopping: "no" } : current) },
      { id: "easier", label: "改用更简单的做法", placement: "primary", onClick: () => {
  ...
  if (sharedTensionOpen && sharedTensionRequired) {
    ...
    return <section className="rm-page">
      ...
      <ActionRail actions={[
        { id: "back-motion", label: "返回活动检查", placement: "secondary", onClick: () => setSharedTensionOpen(false) },
        { id: "confirm-result", label: focusedReassessmentActive ? "确认复查结果" : "查看评估结果", placement: "primary", disabled: !sharedTensionComplete, onClick: () => {
  ...
  if (assessmentSummaryOpen && assessmentReadyForTreatment) {
    ...
    return <section className="rm-page rm-assessment-summary">
      ...
      <ActionRail actions={assessmentNeedsReferral ? [
        { id: "review-checks", label: "查看 / 修改检查", placement: "secondary", onClick: () => { setAssessmentSummaryOpen(false); if (sharedTensionRequired) setSharedTensionOpen(true); } },
        { id: "save-medical", label: "保存并结束本次", placement: "primary", onClick: () => saveRecord("待医学评估") },
      ] : [
        { id: "review-checks", label: "查看 / 修改检查", placement: "secondary", onClick: () => { setAssessmentSummaryOpen(false); if (sharedTensionRequired) setSharedTensionOpen(true); } },
        { id: "continue", label: "评估完成，继续", placement: "primary", onClick: () => { setTrialTargetIndex(0); setCandidateIndex(0); setPostScore(0); setPostScoreConfirmed(false); setPostDiscomfort(""); setTransitionTarget("treatment"); window.scrollTo({ top: 0, behavior: "smooth" }); } },
      ]} />
    </section>;
  }
  const record = assessmentResults[item.id] ?? {};
  ...
  if (item.id === PATELLA_GROUP_PRIMARY_ID) {
    ...
    return <section className="rm-page">
      ...
      <ActionRail actions={visibleAssessmentIndex < assessmentDisplayItems.length - 1 ? [
        { id: "prev", label: "上一个检查", placement: "secondary", onClick: () => visibleAssessmentIndex === 0 ? goToStep(1) : setAssessmentIndex(visibleAssessmentIndex - 1) },
        { id: "next", label: "下一个检查", placement: "primary", disabled: !itemComplete, onClick: () => setAssessmentIndex(visibleAssessmentIndex + 1) },
      ] : sharedTensionRequired ? [
        { id: "prev", label: "上一个检查", placement: "secondary", onClick: () => visibleAssessmentIndex === 0 ? goToStep(1) : setAssessmentIndex(visibleAssessmentIndex - 1) },
        { id: "shared-tension", label: "检查相关肌肉", placement: "primary", disabled: !assessmentComplete, onClick: () => { setSharedTensionOpen(true); window.scrollTo({ top: 0, behavior: "smooth" }); } },
      ] : [
        { id: "prev", label: "上一个检查", placement: "secondary", onClick: () => visibleAssessmentIndex === 0 ? goToStep(1) : setAssessmentIndex(visibleAssessmentIndex - 1) },
        { id: "summary", label: "查看评估结果", placement: "primary", disabled: !assessmentReadyForTreatment, onClick: openAssessmentSummary },
      ]} />
    </section>;
  }
  ...
  return <section className="rm-page">
    ...
    <ActionRail actions={focusedReassessmentActive ? [
      { id: "prev-review", label: focusedAssessmentPosition > 0 ? "上一个复查" : "返回异常反应", placement: "secondary", onClick: () => {
      ...
      { id: "confirm-review", label: focusedAssessmentPosition >= focusedAssessmentIds.length - 1 ? "确认复查结果" : "确认，检查下一项", placement: "primary", disabled: !itemComplete || focusedAssessmentPosition < 0, onClick: () => confirmFocusedAssessment(item.id) },
    ] : visibleAssessmentIndex < assessmentDisplayItems.length - 1 ? [
      { id: "prev", label: visibleAssessmentIndex === 0 ? "返回关键确认" : "上一个检查", placement: "secondary", onClick: () => {
        if (visibleAssessmentIndex === 0) goToStep(1);
        else setAssessmentIndex(visibleAssessmentIndex - 1);
      } },
      { id: "next", label: "下一个检查", placement: "primary", disabled: !itemComplete, onClick: () => setAssessmentIndex(visibleAssessmentIndex + 1) },
