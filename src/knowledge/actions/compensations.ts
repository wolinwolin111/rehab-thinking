 ...
 * 归类逻辑认编号不认文字；未入库部位（旧表）的文字不在词表内，原样透传。
 */
export type CompensationOption = { label: LocalizedText; legacy: string[] };

export const COMPENSATION_OPTIONS: Record<string, CompensationOption> = {
  "knee-height-diff": { label: { plain: "两边膝盖高度不一样", pro: "双侧膝高度不等" }, legacy: ["两边膝盖高度不一样"] },
  "knee-valgus": { label: { plain: "膝盖明显向内偏", pro: "膝动态外翻" }, legacy: ["膝盖明显向内偏"] },
  "knee-inward": { label: { plain: "膝盖向内偏", pro: "膝内偏" }, legacy: ["膝盖向内偏"] },
  "land-knee-inward": { label: { plain: "落地时膝盖明显向内偏", pro: "落地膝内偏" }, legacy: ["落地时膝盖明显内扣"] },
  "heel-early-rise": { label: { plain: "脚跟提前抬起", pro: "足跟提前离地" }, legacy: ["脚跟提前抬起"] },
  "body-sway": { label: { plain: "身体明显晃动", pro: "躯干摇摆" }, legacy: ["身体明显晃动"] },
  "side-balance-worse": { label: { plain: "一侧明显比另一侧更难站稳", pro: "一侧平衡明显更差" }, legacy: ["不舒服的那边明显更难站稳", "不舒服的一侧更难站稳"] },
  "pelvis-tilt": { label: { plain: "骨盆明显歪向一边", pro: "骨盆侧倾" }, legacy: ["骨盆明显歪斜"] },
  "arch-collapse": { label: { plain: "足弓塌下", pro: "足弓塌陷" }, legacy: ["足弓明显塌下", "足弓塌下"] },
  ...
  "side-raise-lower": { label: { plain: "一侧的抬起高度明显更低", pro: "一侧提踵高度更低" }, legacy: ["不舒服的那边抬起高度更低", "患侧抬起高度更低"] },
  ...
  "pelvis-drop": { label: { plain: "骨盆往一边掉", pro: "骨盆下沉" }, legacy: ["骨盆下沉", "骨盆一侧下沉"] },
  ...
  "push-off-weak": { label: { plain: "蹬地使不上劲", pro: "蹬地力量不足" }, legacy: ["蹬地不足"] },
  "calf-symptom-walk": { label: { plain: "走路时小腿明显不舒服", pro: "步行期小腿症状" }, legacy: ["走路时小腿症状明显"] },
  "toe-grip": { label: { plain: "脚趾抓地", pro: "足趾抓地代偿" }, legacy: ["脚趾抓地"] },
