...
export const THIGH_LOCAL_REGION: FullRegion = {
  ...
  candidateGroups: [
    { id: "thigh-front-local", title: "大腿前侧局部问题", match: { locations: ["大腿前"], feelings: ["酸", "紧", "扯", "痛", "无力"], actions: ["踢", "跑", "弯膝", "起身", "下蹲"] }, note: "不自动展开膝关节流程。", candidates: [candidate("thigh-front-release", "大腿前侧轻柔松解", "muscle", "非急性期，在明确紧张而不是损伤中心的位置轻柔松解60～90秒。", "刚发生的拉伤、明显淤青或刺痛时不压疼痛中心。", "复测拉长、发力和原不适动作。", ["thigh-front", "quadriceps", "rectus-femoris"], frontThigh), candidate("thigh-front-control", "大腿前侧低负荷发力", "control", "仰卧绷紧大腿，或坐着把小腿慢慢抬起来一点，保持5秒，做5次。", "不明显增加症状。", "记录完成度和次日反应。", ["thigh-front", "quadriceps"], frontThigh)] },
    { id: "thigh-back-local", title: "大腿后侧局部问题", match: { locations: ["大腿后"], feelings: ["酸", "紧", "扯", "痛", "无力"], actions: ["跑", "冲刺", "弯腰", "迈步", "屈膝"] }, note: "先确认局部拉长与收缩是否复现。", candidates: [candidate("thigh-back-release", "大腿后侧轻柔松解", "muscle", "非急性期，在明确紧张而不是损伤中心的位置轻柔松解60～90秒。", "刚发生的拉伤不强拉、不重压。", "复测拉长、发力和原不适动作。", ["thigh-back", "hamstring"], backThigh), candidate("thigh-back-control", "大腿后侧低负荷发力", "control", "仰卧屈膝，脚跟踩地，把臀部慢慢抬起来停1秒再放下，做2组，每组6～8次。", "不追求强度，次日不持续加重。", "记录发力和走路反应。", ["thigh-back", "hamstring"], backThigh)] },
    { id: "thigh-medial-local", title: "大腿内侧局部问题", match: { locations: ["大腿内"], feelings: ["酸", "紧", "扯", "痛", "无力"], actions: ["夹腿", "变向", "侧移", "跑", "打开腿"] }, note: "以局部内收肌拉长和发力为主。", candidates: [candidate("thigh-medial-release", "大腿内侧轻柔松解", "muscle", "非急性期，在明确紧张的内收肌区域轻柔松解60～90秒。", "避开腹股沟深处和刺痛点。", "复测拉长、夹腿发力和原动作。", ["thigh-medial", "adductor"], medialThigh), candidate("thigh-medial-control", "大腿内侧低负荷发力", "control", "两膝夹软枕，轻轻夹住5秒，做5次。", "不在刺痛范围用力。", "记录发力和走路反应。", ["thigh-medial", "adductor"], medialThigh)] },
    { id: "thigh-lateral-local", title: "大腿外侧局部问题", match: { locations: ["大腿外"], feelings: ["酸", "紧", "扯", "痛", "无力"], actions: ["走", "跑", "单腿", "侧卧", "抬腿"] }, note: "没有相邻关节主诉时不扩大检查。", candidates: [candidate("thigh-lateral-release", "大腿外侧轻柔松解", "muscle", "轻柔处理检查到紧张的前外侧或后外侧肌肉60～90秒。", "不沿髂胫束整条重压。", "复测活动、发力和原动作。", ["thigh-lateral", "tfl", "glute-med"], lateralThigh), candidate("thigh-lateral-control", "大腿外侧低负荷发力", "control", "扶墙小幅向外抬腿，做2组，每组6～8次。", "两侧胯保持一样高，身体不侧倒。", "记录走路和单腿支撑反应。", ["thigh-lateral", "hip-abduction"], lateralThigh)] },
  ],
  exercises: [
    ...
    exercise("thigh-hip-hinge", "站立屈髋（臀部向后）", 3, "3组", "每组8～10个", "背对墙站立约一脚距离，臀部向后移动轻碰墙面，躯干随之向前，再用臀腿力量站直；熟练后离开墙面做。", "脊柱保持稳定，髋膝踝方向自然；碰不到墙说明做成了弯腰。", "减小下移幅度，扶支撑。", "增加轻负重。", ["thigh-back", "hip-hinge", "daily", "glute"], "站立", "学会用髋分担大腿后侧的压力，弯腰搬东西不再靠拉伤过的肌肉硬扛。"),
...
export const CALF_LOCAL_REGION: FullRegion = {
  ...
  candidateGroups: [
    { id: "calf-front-local", title: "小腿前侧局部问题", match: { locations: ["小腿前", "胫骨前"], feelings: ["酸", "紧", "胀", "痛", "无力"], actions: ["勾脚", "走", "跑", "抬脚"] }, note: "不展开踝四方向。", candidates: [candidate("calf-front-release", "小腿前侧轻柔松解", "muscle", "在肌肉区域轻柔松解60～90秒。", "避开胫骨骨面、肿胀中心和刺痛点。", "复测勾脚、发力和原动作。", ["calf-front", "tibialis-anterior"], frontCalf), candidate("calf-front-control", "小腿前侧主动控制", "control", "主动勾脚，保持1～2秒，做2组，每组6～10次。", "不只抬脚趾。", "记录走路抬脚反应。", ["calf-front", "dorsiflexion"], frontCalf)] },
    { id: "calf-back-local", title: "小腿后侧局部问题", match: { locations: ["小腿后", "腓肠肌"], feelings: ["酸", "紧", "扯", "痛", "无力"], actions: ["提踵", "蹬地", "走", "跑", "跳"] }, note: "用伸膝与屈膝位区分负荷。", candidates: [candidate("calf-back-release", "小腿后侧轻柔松解", "muscle", "非急性期，在紧张而不是损伤中心的位置轻柔松解60～90秒。", "刚拉伤或淤青时不重压、不强拉。", "复测两种膝位、提踵和原动作。", ["calf-back", "gastrocnemius", "soleus"], backCalf), candidate("calf-back-control", "小腿后侧低负荷发力", "control", "先坐着或双脚站着轻轻踮脚尖，做2组，每组6～8次。", "次日不持续加重。", "记录高度、个数和走路反应。", ["calf-back", "heel-raise"], backCalf)] },
    { id: "calf-medial-local", title: "小腿内侧局部问题", match: { locations: ["小腿内"], feelings: ["酸", "紧", "扯", "痛", "无力"], actions: ["内翻", "足弓", "提踵", "走", "跑"] }, note: "避免深压内踝后方。", candidates: [candidate("calf-medial-release", "小腿内侧轻柔松解", "muscle", "在小腿内侧肌肉区域轻柔松解60～90秒。", "不深压内踝后方。", "复测内翻、发力和原动作。", ["calf-medial", "tibialis-posterior"], medialCalf), candidate("calf-medial-control", "小腿内侧主动控制", "control", "坐着，脚掌小幅向内转再转回，做2组，每组6～10次；再站着做脚趾抓毛巾。", "小腿不跟着转。", "记录走路和提踵反应。", ["calf-medial", "inversion"], medialCalf)] },
    { id: "calf-lateral-local", title: "小腿外侧局部问题", match: { locations: ["小腿外"], feelings: ["酸", "紧", "扯", "痛", "无力"], actions: ["外翻", "走", "跑", "单腿", "崴"] }, note: "大腿外侧链只作有明确反应时的次级候选。", candidates: [candidate("calf-lateral-release", "小腿外侧轻柔松解", "muscle", "在腓骨外侧肌肉区域轻柔松解60～90秒。", "避开腓骨骨面和刺痛点。", "复测外翻、发力和原动作。", ["calf-lateral", "peroneal"], lateralCalf), candidate("calf-lateral-control", "小腿外侧主动控制", "control", "主动把脚掌小幅向外转，做2组，每组6～10次。", "小腿和膝盖不跟着转。", "记录走路和单腿反应。", ["calf-lateral", "eversion"], lateralCalf), candidate("calf-anterolateral-thigh-lateral-response", "大腿外侧链反应试验", "muscle", "仅在小腿局部处理变化不明显且走路、跑步或单腿动作存在问题时，轻柔处理检查到紧张的大腿前外侧区域60～90秒。", "不沿髂胫束整条重压；这是下一来源反应实验，不直接认定病因。", "只复测原来的小腿主诉动作和仍异常的局部方向。", ["calf-anterolateral", "thigh-lateral", "tfl", "response-test"], ["calf-dorsiflexion", "calf-eversion"]) ] },
  ],
  exercises: [
