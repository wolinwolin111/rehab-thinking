...
const exercise = (id: string, title: string, stage: FullExercise["stage"], sets: string, reps: string, how: string, observe: string, easier: string, harder: string, tags: string[], startPosition: FullExercise["startPosition"]): FullExercise => ({ id, title, stage, sets, reps, how, observe, easier, harder, tags, startPosition });
...
export const THIGH_LOCAL_REGION: FullRegion = {
  ...
  directions: [
    ...
    assessment("thigh-lateral-load", "大腿外侧拉长检查", "direction", "仰卧，把一条腿缓慢向身体中线靠近，骨盆保持不动；左右各做一次。", "比较范围，大腿外侧是否出现熟悉的牵扯或疼痛。", ["thigh-lateral", "hip-adduction", "tfl"]),
  ],
  strengths: [
  ...
  candidateGroups: [
    { id: "thigh-front-local", title: "大腿前侧局部问题", match: { locations: ["大腿前"], feelings: ["酸", "紧", "扯", "痛", "无力"], actions: ["踢", "跑", "弯膝", "起身", "下蹲"] }, note: "不自动展开膝关节流程。", candidates: [candidate("thigh-front-release", "大腿前侧轻柔松解", "muscle", "非急性期，在明确紧张而不是损伤中心的位置轻柔松解30～60秒。", "刚发生的拉伤、明显淤青或刺痛时不压疼痛中心。", "复测拉长、发力和原不适动作。", ["thigh-front", "quadriceps", "rectus-femoris"], frontThigh), candidate("thigh-front-control", "大腿前侧低负荷发力", "control", "仰卧绷紧大腿或坐位小幅伸膝，保持5秒，做5次。", "不明显增加症状。", "记录完成度和次日反应。", ["thigh-front", "quadriceps"], frontThigh)] },
    { id: "thigh-back-local", title: "大腿后侧局部问题", match: { locations: ["大腿后"], feelings: ["酸", "紧", "扯", "痛", "无力"], actions: ["跑", "冲刺", "弯腰", "迈步", "屈膝"] }, note: "先确认局部拉长与收缩是否复现。", candidates: [candidate("thigh-back-release", "大腿后侧轻柔松解", "muscle", "非急性期，在明确紧张而不是损伤中心的位置轻柔松解30～60秒。", "刚发生的拉伤不强拉、不重压。", "复测拉长、发力和原不适动作。", ["thigh-back", "hamstring"], backThigh), candidate("thigh-back-control", "大腿后侧低负荷发力", "control", "脚跟踩地向后轻拉并保持5秒，做5次；稳定后再进阶臀桥。", "不追求强度，次日不持续加重。", "记录发力和走路反应。", ["thigh-back", "hamstring"], backThigh)] },
    ...
    { id: "thigh-lateral-local", title: "大腿外侧局部问题", match: { locations: ["大腿外"], feelings: ["酸", "紧", "扯", "痛", "无力"], actions: ["走", "跑", "单腿", "侧卧", "抬腿"] }, note: "没有相邻关节主诉时不扩大检查。", candidates: [candidate("thigh-lateral-release", "大腿外侧轻柔松解", "muscle", "轻柔处理检查到紧张的前外侧或后外侧肌肉30～60秒。", "不沿髂胫束整条重压。", "复测活动、发力和原动作。", ["thigh-lateral", "tfl", "glute-med"], lateralThigh), candidate("thigh-lateral-control", "大腿外侧低负荷发力", "control", "扶墙小幅向外抬腿，做2组，每组6～8次。", "骨盆不歪，身体不侧倒。", "记录走路和单腿支撑反应。", ["thigh-lateral", "hip-abduction"], lateralThigh)] },
  ],
  exercises: [
    exercise("thigh-front-isometric", "大腿前侧绷紧保持", 1, "2组", "每组5次，每次5秒", "仰卧或坐稳，把膝盖绷直，让大腿前侧收紧后放松。", "两侧发力接近，症状不逐次增加。", "减小用力或保持3秒。", "增加到每组8次。", ["thigh-front", "quadriceps", "isometric"], "仰卧"),
    exercise("thigh-front-extension-control", "坐姿伸膝控制", 2, "3组", "每组8～10个", "坐稳，缓慢把小腿抬起到舒适范围，再慢慢放下。", "膝盖朝前，抬起和放下都能控制。", "减小抬起范围。", "增加停留或轻阻力。", ["thigh-front", "quadriceps", "knee-extension"], "坐位"),
    exercise("thigh-back-isometric", "脚跟后拉保持", 1, "2组", "每组5次，每次5秒", "坐稳，脚跟踩地，像要把脚跟拖向椅子下方，但脚不要真的移动。", "大腿后侧均匀发力，不抽筋。", "减小用力或保持3秒。", "增加到每组8次。", ["thigh-back", "hamstring", "isometric"], "坐位"),
    exercise("thigh-bridge", "臀桥", 2, "3组", "每组8～10个", "仰卧屈膝，脚跟踩地，抬起臀部再缓慢落下。", "骨盆不歪，腰部不过度顶起。", "减小高度。", "进阶单腿臀桥。", ["thigh-back", "hamstring", "glute"], "仰卧"),
    exercise("thigh-medial-isometric", "夹枕保持", 1, "2组", "每组5次，每次5秒", "仰卧屈膝，在两膝之间放软枕，轻轻夹住后放松。", "大腿内侧均匀发力，不出现刺痛。", "减小夹力或保持3秒。", "增加到每组8次。", ["thigh-medial", "adductor", "isometric"], "仰卧"),
    exercise("thigh-medial-active", "侧向收腿控制", 2, "3组", "每组8～10个", "站稳并扶住固定物，把一侧腿从外侧缓慢收回身体中线。", "骨盆不转，收回过程平稳。", "减小移动范围。", "增加轻阻力。", ["thigh-medial", "adductor", "control"], "站立"),
    exercise("thigh-lateral-isometric", "侧向抬腿保持", 1, "2组", "每组5次，每次5秒", "扶墙站稳，把腿小幅向外抬起并保持，脚尖朝前。", "躯干不侧倒，骨盆不歪。", "减小抬腿范围或时间。", "增加到每组8次。", ["thigh-lateral", "glute-med", "isometric"], "站立"),
    exercise("thigh-lateral-stability", "单腿骨盆稳定", 2, "3组", "每侧20～30秒", "扶住固定物单腿站立，保持两侧骨盆大致水平。", "支撑腿稳定，身体不明显侧倒。", "双手扶稳并缩短时间。", "减少扶持或加入轻微重心移动。", ["thigh-lateral", "glute-med", "single-leg"], "站立"),
    exercise("thigh-hip-hinge", "站立屈髋", 3, "3组", "每组8～10个", "双脚站稳，臀部向后移动，躯干随之向前，再用臀腿力量站直。", "脊柱保持稳定，髋膝踝方向自然。", "减小下移幅度。", "增加轻负重。", ["thigh-back", "hip-hinge", "daily"], "站立"),
    exercise("thigh-sit-stand", "坐下再站起", 3, "3组", "每组8～10个", "从稳固椅子缓慢坐下，再均匀用双腿站起。", "左右用力接近，膝盖朝向脚尖。", "提高椅子并扶手。", "降低椅子或轻负重。", ["thigh-front", "thigh-medial", "sit-to-stand", "daily"], "站立"),
    exercise("thigh-lateral-step", "侧向迈步", 3, "3组", "每侧8～10步", "膝髋微屈，向侧方小步移动，再缓慢回到起点。", "骨盆保持稳定，双脚不过度内扣。", "缩小步幅并扶稳。", "增加轻阻力带。", ["thigh-lateral", "thigh-medial", "lateral-step"], "站立"),
    exercise("thigh-step", "台阶与单腿控制", 4, "3组", "每侧6～8个", "从低台阶上台开始，再练慢速下台。", "症状可控，骨盆和膝足稳定。", "降低台阶。", "增加高度。", ["step", "single-leg"], "站立"),
    exercise("thigh-run-return", "跑步与变速回归", 5, "3～4组", "每组20～30秒", "从走跑交替逐步增加连续跑，再进入加速或变向。", "无锐痛，动作稳定，次日反应可接受。", "缩短慢跑。", "一次只增加速度、时间或变向中的一项。", ["run", "sport"], "站立"),
  ],
};
...
export const CALF_LOCAL_REGION: FullRegion = {
  ...
  directions: [
    assessment("calf-dorsiflexion", "踝背屈检查", "direction", "坐稳，脚跟放地，慢慢把脚背向小腿方向勾起。", "比较范围；记录小腿前侧缩短或小腿后侧拉长时是否出现熟悉症状。", ["calf-front", "calf-back", "dorsiflexion"]),
    assessment("calf-plantarflexion", "踝跖屈检查", "direction", "坐稳，小腿放松，慢慢把脚背向下压到舒适范围。", "比较范围；记录小腿后侧缩短或小腿前侧拉长时是否出现熟悉症状。", ["calf-front", "calf-back", "plantarflexion", "gastrocnemius", "soleus"]),
    assessment("calf-inversion", "踝内翻检查", "direction", "坐稳，小腿不动，把脚掌缓慢向内转。", "比较范围；记录小腿内侧缩短或小腿外侧拉长时是否出现熟悉症状。", ["calf-medial", "calf-lateral", "inversion"]),
    assessment("calf-eversion", "踝外翻检查", "direction", "坐稳，小腿不动，把脚掌缓慢向外转。", "比较范围；记录小腿外侧缩短或小腿内侧拉长时是否出现熟悉症状。", ["calf-medial", "calf-lateral", "eversion"]),
  ],
  strengths: [
  ...
  exercises: [
    exercise("calf-front-active", "主动勾脚", 1, "2组", "每组6～10个", "坐稳，脚跟放地，把脚背向小腿方向勾起，再缓慢放下。", "脚背整体抬起，不只抬脚趾。", "减小范围。", "增加到每组15个。", ["calf-front", "dorsiflexion"], "坐位"),
    exercise("calf-front-endurance", "脚跟走耐力", 2, "3组", "每组10～15步", "扶稳后轻抬前脚掌，用脚跟小步向前走。", "脚尖能持续抬起，不拖地。", "原地交替勾脚。", "增加步数。", ["calf-front", "dorsiflexion", "gait"], "站立"),
    exercise("calf-back-seated-raise", "坐姿提踵", 1, "3组", "每组8～12个", "坐稳，前脚掌踩地，缓慢抬起脚跟再放下。", "小腿后侧发力，节奏稳定。", "减小高度或个数。", "膝上增加轻负重。", ["calf-back", "soleus", "heel-raise"], "坐位"),
    exercise("calf-back-standing-raise", "双脚站姿提踵", 2, "3组", "每组8～12个", "扶墙站立，两侧脚跟同时抬起，再缓慢落下。", "两侧高度接近，不向内外歪。", "回到坐姿提踵。", "进阶单脚提踵。", ["calf-back", "gastrocnemius", "heel-raise"], "站立"),
    exercise("calf-medial-active", "小幅内翻控制", 1, "2组", "每组6～10个", "坐稳，小腿不动，把脚掌小幅向内转，再缓慢回正。", "动作来自脚踝，膝盖不跟着转。", "减小范围。", "用另一只脚增加轻阻力。", ["calf-medial", "inversion"], "坐位"),
    exercise("calf-medial-arch", "足弓保持与提踵", 2, "3组", "每组8～10个", "站稳，保持足弓和大脚趾根部接触地面，再做小幅提踵。", "足弓不塌，脚跟不向外甩。", "只练足弓保持。", "增加提踵高度。", ["calf-medial", "arch", "heel-raise"], "站立"),
    exercise("calf-lateral-active", "小幅外翻控制", 1, "2组", "每组6～10个", "坐稳，小腿不动，把脚掌小幅向外转，再缓慢回正。", "膝盖不跟着转，动作不靠甩动。", "减小范围。", "增加轻阻力。", ["calf-lateral", "eversion"], "坐位"),
    exercise("calf-lateral-stability", "单腿外侧稳定", 2, "3组", "每侧20～30秒", "扶稳后单腿站立，保持足底三点支撑和脚踝稳定。", "脚踝不反复向外翻，身体不过度摇晃。", "双手扶稳并缩短时间。", "减少扶持或轻微移动重心。", ["calf-lateral", "eversion", "single-leg"], "站立"),
    exercise("calf-gait", "重心转移与走路", 3, "3组", "每组10次，再走10米", "练习脚跟着地、身体前移和前脚掌蹬地。", "步幅自然，不跛行。", "扶桌小幅转移。", "增加步数或速度。", ["gait"], "站立"),
    exercise("calf-step-single-leg", "台阶与单腿稳定", 4, "3组", "每侧6～8个", "先单腿站，再做低台阶上下。", "症状可控，髋膝踝协同。", "扶墙并降低台阶。", "增加高度。", ["step", "single-leg"], "站立"),
    exercise("calf-run-hop", "跑跳与蹬地回归", 5, "3～4组", "每组5次或20秒", "从走跑交替、快提踵进入小幅双脚跳。", "无锐痛，次日反应可接受。", "回到快走。", "一次只增加一项负荷。", ["run", "hop"], "站立"),
  ],
};
