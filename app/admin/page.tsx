    ...
    <div className="admin-workspace">
      ...
      <section className="admin-detail" aria-label="案例详情">
        {!detail ? <div className="admin-empty-detail"><strong>选择一个案例</strong><span>查看康复记录、时间线、反馈和版本。</span></div> : <>
          ...
          <dl className="admin-facts"><div><dt>当前阶段</dt><dd>{detail.caseRecord.currentStage ?? "未记录"}</dd></div><div><dt>康复记录</dt><dd>{detail.caseRecord.sessionCount} 次</dd></div><div><dt>快照修订</dt><dd>{detail.snapshot.revision}</dd></div><div><dt>问题线程</dt><dd>{typeof (detail.snapshot.payload.identity as Record<string, unknown> | undefined)?.problemThreadId === "string" ? String((detail.snapshot.payload.identity as Record<string, unknown>).problemThreadId) : "未记录"}</dd></div><div><dt>当前会话</dt><dd>{typeof (detail.snapshot.payload.identity as Record<string, unknown> | undefined)?.sessionId === "string" ? String((detail.snapshot.payload.identity as Record<string, unknown>).sessionId) : "未记录"}</dd></div><div><dt>来源渠道</dt><dd>{SOURCE_LABELS[detail.caseRecord.sourceChannel ?? "unknown"] ?? detail.caseRecord.sourceChannel}{detail.caseRecord.sourceDetail ? `（${detail.caseRecord.sourceDetail}）` : ""}</dd></div>{detail.caseRecord.isTestCase ? <><div><dt>测试场景</dt><dd>{detail.caseRecord.scenarioId ?? "未记录"}</dd></div><div><dt>运行批次</dt><dd>{detail.caseRecord.testRunId ?? "未记录"}</dd></div></> : null}<div><dt>应用版本</dt><dd>{detail.caseRecord.appVersion}</dd></div><div><dt>知识 / 决策</dt><dd>{detail.caseRecord.knowledgeVersion} / {detail.caseRecord.decisionVersion}</dd></div></dl>
